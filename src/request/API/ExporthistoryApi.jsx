...
// import { resetAddStatus } from "@/redux/Exporthistory/UpdateStatus/UpdateStatus";
// import { resetSessionStatus } from "@/redux/Exporthistory/UpdateStatus/UpdateSession";
// import { resetStatus } from "@/redux/Exporthistory/extendTask/AddExtendPlan";
...
const ExporthistoryApi = () => {
  ...
  const AddExtendPlanStateStatus = useSelector(
  ...
  useEffect(() => {
    if (entityid && listid && dataSetID) {
      ...
      async function getlistdetails() {
        const response = await request.post(
          API_TEST_URL,
          "arc_export/getexporthistorylistdetails",
          postRequest
        );
  ...
  useEffect(() => {
    ...
    if (
      ...
      UpdateSessionState === "successful" ||
      AddExtendPlanStateStatus === "successful"
    ) {
      async function getlistdetails() {
        const response = await request.post(
          API_TEST_URL,
    ...
    } else if (UpdateStatus === "failed" || UpdateSessionState === "failed") {
      dispatch(setLoading(false));
    }
  }, [UpdateStatus, UpdateSessionState, AddExtendPlanStateStatus]);
...
export default ExporthistoryApi;
