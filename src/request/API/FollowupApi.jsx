...
const FollowupApi = (frompage, followuprequestclass) => {
  const dispatch = useDispatch();
  const addStatus = useSelector(
    (state) => state.UpdateStatusfollowState.Addstatus
  );
  ...
  const Addfollowup = useSelector(
    (state) => state.AddFollowuplistdetailsState.Addstatus
  );

  const updatefollowup = useSelector(
    (state) => state.UpdateStatusfollowState.Addstatus
  );
  ...
  useEffect(() => {
    if (entityid && listid && dataSetID) {
      ...
      dispatch(setLoading(true));
      postRequest;
      console.log(frompage.followuprequestclass);
      ...
      async function getlistdetails() {
        const response = await request.post(
          API_TEST_URL,
          "planfollowup/getfollowuplistdetails",
          frompage.frompage === "360details"
            ? frompage.followuprequestclass
            : postRequest
        );
  ...
  }, [postRequest, addStatus, Addfollowup, updatefollowup]);
