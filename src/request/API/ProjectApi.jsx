...
const ProjectApi = () => {
  ...
  const AddProjectStatus = useSelector((state) => state.arcAddProject.status);
  const UpdateMasterTaskStateStatus = useSelector(
    (state) => state.defaultInsertUpdate.status
  );
  ...
  useEffect(() => {
    if (entityid && listid && dataSetID) {
      console.log(AddProjectStatus);
      dispatch(setLoading(true));
      async function getlistdetails() {
        const response = await request.post(
          API_TEST_URL,
          "ArcProject/GetProjectListDetails",
          postRequest
        );
  ...
  const addTaskStateStatus = useSelector(
    (state) => state.addTaskState.Addstatus
  );
  ...
  useEffect(() => {
    ...
    if (
      ...
      UpdateMasterTaskStateStatus === "success" ||
      UpdateMasterTaskStateStatus === "failed" ||
      addTaskStateStatus === "succeeded" ||
      addTaskStateStatus === "failed"
    ) {
      dispatch(setLoading(true));
      async function getlistdetails() {
        const response = await request.post(
  ...
  }, [AddProjectStatus, UpdateMasterTaskStateStatus, addTaskStateStatus]);
