...
import { resetStatus as subtaskupdatereset } from "@/redux/Execution/addSubTask/AddSubTaskData";
...
const ExecutionApi = () => {
  ...
  useEffect(() => {
    if (entityid && listid && dataSetID) {
      ...
      async function getlistdetails() {
        const response = await request.post(
          API_TEST_URL,
          "planexecution/getplanexeclistdetails",
          postRequest
        );
  ...
  useEffect(() => {
    ...
    if (
      AddSubTaskPlanStateStatus === "successful" ||
