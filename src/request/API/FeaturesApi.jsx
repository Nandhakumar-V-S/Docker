...
import { resetInsertUpdateStatus } from "@/redux/DefaultInsertUpdate/DefaultInsertUpdate";

const FeaturesApi = (page, featureproj360) => {
  const dispatch = useDispatch();
  const dataSetID = useSelector(dataSetIDInfo);
  let postRequest = useSelector(postDataInfo);
  ...
  useEffect(() => {
    if (entityid && listid && dataSetID) {
      ...
      // Assuming postRequest is an array
      let updatedpostRequest = {};
      updatedpostRequest = {
        ...postRequest,
        transactionid:
          sessionStorage.getItem("Current_EntityID") === ""
            ? ""
            : sessionStorage.getItem("Current_EntityID"),
      };
  ...
  useEffect(() => {
    ...
    if (DefaultStatus === "success") {
      async function getlistdetails() {
        ...
        dispatch(getListdataSuccess(response));
        dispatch(resetInsertUpdateStatus());
        dispatch(setLoading(false));
      }
      getlistdetails();
    }
