...
const PlanApi = () => {
  ...
  const insertData = useSelector(insertDataInfo);
  ...
  useEffect(() => {
    const clientOffset = new Date().getTimezoneOffset(); //  minutes
  ...
  useEffect(() => {
    if (dataSetID) {
      ...
      async function getDataset() {
        ...
        sessionStorage.setItem("Exec_dataSetID", dataSetID);
      }
  ...
  useEffect(() => {
    if (entityid && listid && dataSetID) {
      ...
      async function getlistdetails() {
        const response = await request.post(
          API_TEST_URL,
          "plan/getplanlistdetails",
          postdatawithtimezone
        );
  ...
  useEffect(() => {
    dispatch(setLoading(true));
    console.log(UpdateStatus);
    if (UpdateStatus === "successful" || UpdateSessionState === "successful") {
      async function getlistdetails() {
        const postdatawithtimezone = {
          ...postRequest,
          totalOffsetMinutes: offset,
  ...
  }, [UpdateStatus, UpdateSessionState]);
  ...
  useEffect(() => {
    if (insertData.apiname !== "" && insertData.transactionid !== "") {
      async function insertplanassignments() {
        const response = await request.post(
          API_TEST_URL,
          "plan/insertplanassignments",
          insertData
        );
        if (response) {
          console.log(response);
          dispatch(resetInsertData());
        }
      }
      insertplanassignments();
    }
  }, [insertData]);
