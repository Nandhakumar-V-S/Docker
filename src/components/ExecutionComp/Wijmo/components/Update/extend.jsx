...
import { AddExtendPlanData } from "@/redux/Execution/extendTask/AddExtendPlan";
import ExtendOffline_API from "./extend.json";
...
export default function ExtendPlan({ show, setShow, SelectedRow }) {
  ...
  // ! Format Date
  // ! Merge data
  const [updatedSession, setUpdatedSession] = useState(ExtendOffline_API);

  useEffect(() => {
    const updateDefaultValues = () => {
      const updatedData = { ...ExtendOffline_API };

      SessionDefaultInputs?.result?.forEach((defaultValue) => {
        ExtendOffline_API?.result.groups.forEach((group) => {
  ...
  // const AddExtendPlanState = useSelector(
  //   (state) => state.AddExtendPlanState.Status
  // );
  const addExtendPlanState = useSelector((state) => state.AddExtendPlanState);

  const AddExtendPlanState = addExtendPlanState.Status;
  const responseData = addExtendPlanState.responseData; // If you need the response data
  const error = addExtendPlanState.error; // If you want to handle any errors
    ...
    LeadFormAPI?.groups.forEach((sectionlist) => {
      sectionlist.fields.forEach((FieldList) => {
        if (FieldList.default_value && FieldList.default_value != null) {
          ...
          if (FieldList.controltype === "date") {
            setInputValues((prevState) => ({
              ...prevState,
              [FieldList.api_name + FieldList.id]:
                (FieldList.default_value &&
                  formatDate(FieldList.default_value)) ||
                format(new Date(), "MM/dd/yyyy") ||
                new Date(),
            }));
          }
    ...
    if (isValid) {
      ...
      dispatch(
        ...
        .then((response) => {
          ...
          // Check the response to decide if the popup should close
          if (response.result.response === "Plan Already Exists in same date") {
            setShow(true); // Show a message or handle accordingly
          }
          if (response.result.response === "successfully Extend") {
            setInputValues({}); // Clear input values
            setAllInputValues([]); // Clear all input values
            setShowErrors(false); // Hide any validation errors
            setShow(false); // Close the popup
          }
        })
        .catch((error) => {
          console.error("Error during save:", error); // Handle any errors
          setShowErrors(true); // Show error state if needed
          setShow(true); // Optionally keep the popup open to show the error
        });
    } else {
      setShowErrors(true);
      setShow(false);
