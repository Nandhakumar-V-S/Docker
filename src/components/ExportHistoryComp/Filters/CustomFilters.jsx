...
export const CustomFilters = ({
  ...
  const renderCustomFilterOptions = (filter) => {
    ...
    else if (filter.filtercontroltype === "datepicker") {
      if (!filter.masterid) {
        return (
        //  <>
        //   <input
        ...
        ClassName=""
        onChange={(date) => handleDateInputChange(date,filter)}
      />
        );
      }
    }
  ...
  const handleCustomFilterSubmit = (selectedOptions, textValues) => {
    if (
      ...
      console.log(selectedOptions, textValues,"selectedOptions, textValues");
  ...
  const handleDateInputChange = (date,filter) => {
    setStartDate(date);
    // setSelectedValue(format(date, "MM/dd/yyyy"));
    // setSelectedOptions(format(date, "MM/dd/yyyy"))
    const formattedVariable = format(date, "MM/dd/yyyy")
    console.log(date,"format(date)",format(date, "MM/dd/yyyy"));

    const structure = {
      "id" :filter.id,
      "api_name":filter.api_name,
      "controltype":filter.filtercontroltype,
      "filterCategory":"additional",
       "selectedValue":[formattedVariable]
    }
    console.log(structure,"structure");    
    setSelectedOptions([structure]);
  };


  const relevantFilters = ["workweek", "workday", "createdon"];
  return (
    <>
      <ArcAlertPopup
