...
// import EditNotes from "./components/Update/EditNotes";
...
// import { masterDataInfo, listDataInfo } from "@/redux/Notes/selector";
...
// import Updatesubtask from "@/modules/NotesModule/components/Updatesubtask/Updatesubtask";
...
//   const { NotesIsGrouping } = useContext(ArcGlobalContextProvider);
//   // const { orderBy, orderByHeader } = useSelector((state) => state.task);
...
//       formid: formid,
//     };
//     dispatch(GetEditNotesdetails(TransactionID));
//   };
...
//                             title="Edit SubTask"
//                             onClick={async () => {
//                               await setsubtaskshow(true);
//                               await dispatch(resetsubTaskDatas());
//                               // setEditTaskshow(true);
//                               // setistageditedEditTask(false);
//                               // setUpdateselectedTagItemEdit([]);
//                               await UpdatesubtaskRowData(RowData);
//                             }}
//                           >
//                             <BiSolidEditAlt />
//                           </span>
...
//             NotesIsGrouping ? "is-grouping" : null
//           }`}
//           headersVisibility="Column"
...
import { GetEditNotesdetails } from "@/modules/NotesModule/components/Addtask/GetEditNotesDetails";
// ~ Selected Filter
import { ListContext } from "@/modules/NotesModule/index";
import { selectedFiltersInfo } from "@/redux/Notes/selector";
// ~ Selected Filter
// ! Import
const WijmoFlexgrid = ({ loading, tableData, sortedColumns }) => {
  const dispatch = useDispatch();
  ...
  console.log(collectionView.items);
  //checkbox
  const { selectedcheckboxId, setSelectedcheckboxId } =
    useContext(SelectedRowContext);
  const { checkboxRowValues, setcheckboxRowValues } =
    useContext(SelectedRowContext);
  const { setSelectedcheckboxValues } = useContext(SelectedRowContext);
  const { bulkupdatevalues, setBulkupdatevalues } =
    useContext(SelectedRowContext);
  useEffect(() => {
    const ids = Object.keys(selectedcheckboxId);
    setBulkupdatevalues(ids);
    setSelectedcheckboxValues(checkboxRowValues);
  }, [selectedcheckboxId]);
  const { setnotesupdateVisible } = useContext(SelectedRowContext);
  console.log(bulkupdatevalues);
  console.log(checkboxRowValues);
  // const handleCheckboxSelect = (e, id) => {
  //   if (e.target.checked) {
  //     setSelectedcheckboxId((prev) => ({ ...prev, [id]: true }));
  //     setnotesupdateVisible(false);
  //   } else {
  //     setSelectedcheckboxId((prev) => {
  //       const { [id]: _, ...rest } = prev;
  //       setnotesupdateVisible(true);
  //       return rest;
  //     });
  //   }
  // };

  const handleCheckboxSelect = (e, id, item) => {
    if (e.target.checked) {
      // Checkbox is checked
      setSelectedcheckboxId((prev) => ({ ...prev, [id]: true }));
      setcheckboxRowValues((prev) => [...prev, { id: id, item }]);
    } else {
      ...
      const unmatchedItems = checkboxRowValues?.filter(
        (item) => item.id !== id
      );
      setcheckboxRowValues(unmatchedItems);
    }
  };
  // Effect to update notesupdateVisible based on selectedcheckboxId
  useEffect(() => {
    if (Object.keys(selectedcheckboxId).length > 0) {
      setnotesupdateVisible(false);
    } else {
      setnotesupdateVisible(true);
    }
  ...
  useEffect(() => {
    if (BulkUpdateProject || BulkUpdateStatus || loading) {
      setSelectedcheckboxId({});
      setBulkupdatevalues([]);
    }
  }, [BulkUpdateStatus, BulkUpdateProject, loading]);
                  ...
                  <div className="checkbox-container">
                    {/* <span>
                      <input
                        type="checkbox"
                        id={id}
                        onChange={(e) => handleCheckboxSelect(e, id, Tilte)}
                    ...
                    <span>
                      <input
                        type="checkbox"
                        id={id}
                        onChange={(e) => handleCheckboxSelect(e, id, ctx.item)}
                        checked={!!selectedcheckboxId[id]}
                        disabled={
                          ctx.item.utbl_notes_column33 == "Processed to project"
                        }
                        className={
                          ctx.item.utbl_notes_column33 == "Processed to project"
                            ? "disabled"
