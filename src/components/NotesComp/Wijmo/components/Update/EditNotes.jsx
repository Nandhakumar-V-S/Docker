...
import { masterDataInfo as masterDataInfoTask } from "@/redux/Notes/selector";
...
import Editjson from "./EditNotes.json";
import { UpdateMasterNotesData } from "@/modules/NotesModule/components/Addtask/EditNotes";
...
function HeaderContent({
  ...
  return (
    <div className="header-content">
      <h3 title="Edit Notes">Edit Notes</h3>
...
function FooterContent({ handleCancelButtonClick, handleSaveButtonClick }) {
  return (
    <div className="footer-content">
      ...
      <button onClick={handleSaveButtonClick} title="Update">
        Update
      </button>
    </div>
  );
...
export default function EditNotes({
  ...
  const TaskDefaultInputs = useSelector(
    (state) => state.GetEditNotesdetailsstate.GetNotesDetails
  );
  ...
  useEffect(() => {
    const updateDefaultValues = () => {
      const updatedData = { ...Editjson };

      const updatedDatas =
        TaskDefaultInputs.length > 0 ? JSON.parse(TaskDefaultInputs) : [];
      updatedDatas?.result?.forEach((defaultValue) => {
        Editjson?.result?.groups?.forEach((group) => {
  ...
  useEffect(() => {
    const updateDefaultValues = () => {
      ...
      TaskDefaultInputs?.result?.forEach((defaultValue) => {
        updatedDatas?.result?.forEach((group) => {
          group.fields?.forEach((field) => {
            if (field.id.toLowerCase() === defaultValue.id.toLowerCase()) {
              if (
  ...
  const LeadFormAPI = Editjson?.result; // Local Json
  // !POST Form Details
  // const [inputValues, setInputValues] = useState({});
  ...
  useEffect(() => {
    SetDefaultValues();
  }, [EditTaskShow, updatedscreenFields, TaskDefaultInputs, updatedSession]);
  ...
  const handleSaveButtonClick = () => {
    ...
    if (isValid) {
      ...
      if (!isEqual(allInputValues, previousInputValues.current)) {
        ...
        dispatch(UpdateMasterNotesData({ allInputValues, TransactionId }));
    ...
    if (taskresponseresult) {
      if (taskresponseresult?.response === "Task Not Exists.") {
        ...
        if (allInputValues.length > 0) {
          ...
          dispatch(UpdateMasterNotesData(requestData));
          setInputValues({});
          setAllInputValues([]);
          setShowErrors(false);
          setEditTaskShow(false);
                ...
                >
                  <div className="add-contact-form add-lead-form">
                    ...
                    <div className="main-content">
                      ...
                      {/* {
                        <pre>
                          {JSON.stringify(SessionDefaultInputs, null, 2)}
                        </pre>
                      } */}
