...
const QuickFilterHeader = ({
  ...
  const [dateMasterId, setDateMasterId] = useState("");
  const [newstartDate, setnewStartDate] = useState();

  console.log(dateMasterId);
  ...
  const HandleDateChange = (date, id, api_name, masterid) => {
    const name = "Imported On";
  ...
  const [planSelectedYear, setPlanSelectedYear] = useState(currentYear);
  const [planSelectedWeek, setPlanSelectedWeek] = useState(currentWeek);
  const [planselectedMonth, setPlanSelectedMonth] = useState(currentMonth);
  const [selectedWeek, setSelectedWeek] = useState(currentWeek);
  const [selectedMonth, setSelectedMonth] = useState(currentMonth);

  console.log(planSelectedWeek);
  console.log(planselectedMonth);
  console.log(planSelectedYear);

  const handleMonthChange = (month, api_name, id, masterid) => {
    const name = "Work Month";
    console.log(month + 1, selectedYear, api_name);
    const selectedValue = [month + 1 + "$" + selectedYear];
    ...
    dispatch(setMonthFilter(id, api_name, name, month + 1, selectedYear));
  };

  const handlePlanMonthChange = (month, api_name, id, masterid) => {
    const name = "Plan Month";
    console.log(month + 1, planSelectedYear, api_name);
    const selectedValue = [month + 1 + "$" + planSelectedYear];
    console.log(selectedValue);
    dispatch(
      addSelectedFilter({
        id,
    ...
    dispatch(
      setPlanMonthFilter(id, api_name, name, month + 1, planSelectedYear)
    );
  };

  const HandlePlanWeekChange = (week, api_name, id, masterid) => {
    const name = "Plan Week";
    console.log(week + 1, planSelectedYear, id, api_name);

    const selectedValue = [week + 1 + "$" + planSelectedYear];
    console.log(selectedValue);
    dispatch(
      addSelectedFilter({
        id,
    ...
    dispatch(setPlanWeekFilter(id, api_name, name, week + 1, selectedYear));
    // dispatch(changeMasterdata(masterid, api_name, week + 1, planSelectedYear));
  };

  const HandleWeekChange = (week, api_name, id, masterid) => {
    const name = "Work Week ";
    ...
    dispatch(setWeekfilter(id, api_name, name, week + 1, planSelectedYear));
          ...
          if (filter.masterid) {
            ...
            if (matchingLookup) {
              ...
              if (newMatchingLookup) {
                ...
                const defaultOptionIds = Array.isArray(
  ...
  useEffect(() => {
    if (planWeekFilter.length > 0) {
      const selectedweek = planWeekFilter[0].week;
      const selectedyear = planWeekFilter[0].year;
      console.log(selectedweek, selectedyear);
      setPlanSelectedWeek(selectedweek);
      setPlanSelectedYear(selectedyear);
    } else {
      setPlanSelectedWeek("");
    }
  }, [planWeekFilter]);
  ...
  useEffect(() => {
    if (planMonthFilter.length > 0) {
      console.log(planMonthFilter);
      const selectedMonth = planMonthFilter[0].month;
      const selectedyear = planMonthFilter[0].year;
      console.log(selectedMonth, selectedyear);
      setPlanSelectedMonth(selectedMonth);
      setPlanSelectedYear(selectedyear);
    } else {
      setPlanSelectedMonth("");
    }
  }, [planMonthFilter]);

  useEffect(() => {
    if (monthFilter.length > 0) {
      const selectedMonth = monthFilter[0].month;
      const selectedyear = monthFilter[0].year;
      console.log(selectedMonth, selectedyear);
  ...
  //added by Gomathi
  useEffect(() => {
    if (datePickerFilter.length > 0) {
      const selectedDate = datePickerFilter[0].formattedDate;
      ...
      return quickFilterData
        ...
        .map((filter) => {
          if (
            ...
            ) : ["planweekfilter"].includes(filter.filtercontroltype) ? (
              <>
                <div className="action-week-filter">
                  <WeekFilterV3
                    displayName={filter.name}
                    ...
                    id={filter.id}
                    masterid={filter.masterid}
                    selectedYear={planSelectedYear}
                    setSelectedYear={setPlanSelectedYear}
                    selectedWeek={planSelectedWeek}
                    setSelectedWeek={setPlanSelectedWeek}
                    HandleWeekChange={HandlePlanWeekChange}
                  />
                </div>
              </>
            ) : ["monthfilter"].includes(filter.filtercontroltype) ? (
              <>
                <div className="action-week-filter">
                  <MonthFilterV2
                    displayName={filter.name}
            ...
            ) : ["planmonthfilter"].includes(filter.filtercontroltype) ? (
              <>
                <div className="action-week-filter">
                  <MonthFilterV2
                    displayName={filter.name}
                    ...
                    selectedYear={planSelectedYear}
                    setSelectedYear={setPlanSelectedYear}
                    selectedMonth={planselectedMonth}
                    setSelectedMonth={setPlanSelectedMonth}
                    handleMonthChange={handlePlanMonthChange}
                  />
                </div>
              </>
            ...
            ) : ["onclickloadmultiselect"].includes(
                filter.filtercontroltype
              ) ? (
              <>
                {/* <pre>{JSON.stringify(filter, null, 2)}</pre> */}
            ...
            //filter.ismapped &&
            (filter.isquickfilter &&
              filter.filtercontroltype === "datepicker") ||
            filter.filtercontroltype === "weekfilter" ||
            filter.filtercontroltype === "monthfilter"
          ) {
            // const filterOptions =
            //   matchedLookup[filter.masterid].mastervalues || [];
