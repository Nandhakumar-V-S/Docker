...
export const CustomFilters = ({
      ...
      payloadMap.forEach((currentAttrId) => {    
        ...
        CurrentAttribute.forEach((id) => {
         const matchedObject = Object.values(matchedLookup).find(
          ...
          if (matchedObject) {
            const { id, api_name, masterid } = matchedObject;
            const value = [];
            dispatch(
              addSelectedFilter({
                id,
  ...
  useEffect(() => {
    if (additionalFilters.length > 0) {
      const updatedOptions = additionalFilters.map((filter) => ({
        id: filter.filterid,
  ...
  const handleTextChange = (filter, value) => {
    const { id, api_name, filtercontroltype } = filter;
  ...
  const handleRemoveItem = (filterId) => {
