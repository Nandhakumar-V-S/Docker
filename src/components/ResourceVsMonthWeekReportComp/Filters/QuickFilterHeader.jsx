...
import {
  ...
  changeMasterdata,
  setMonthFilter,
} from "@/redux/ResourceVsMonthWeekReport/actions";
...
const QuickFilterHeader = ({
  ...
  const [weekMasterid, setWeekMasterid] = useState("");
  const [monthMasterid, setMonthMasterid] = useState("");
  ...
  const handleMonthChange = (month, api_name, id, masterid) => {
    ...
    dispatch(changeMasterdata(masterid, api_name, month + 1, selectedYear));
  };

  const HandleWeekChange = (week, api_name, id, masterid) => {
    const name = "Week Filter";
    console.log(week + 1, selectedYear, id, api_name);
    setWeekMasterid(masterid);
  ...
  const clearFilters = () => {
    ...
    const horGrpby = horizontalGroupBy[0];
    console.log(horGrpby);
    if (horGrpby?.api_name === "workweek_Group") {
      console.log(currentMonth);
      console.log(monthMasterid);
      console.log(currentYear);
      selectedFilterIDs = [
        ...new Set(
          selectedFilters
            .filter((item) => item.apiname !== "workmonth")
            .map((item) => item.filterid)
        ),
        // ...new Set(selectedFilters.map((item) => item.filterid)),
      ];
      dispatch(changeMasterdata(monthMasterid, "", currentMonth, currentYear));
    } else if (horGrpby?.api_name === "workday_Group") {
      if (horGrpby) {
        selectedFilterIDs = [
          ...new Set(
            selectedFilters
              .filter((item) => item.apiname !== "workweek")
              .map((item) => item.filterid)
          ),
          // ...new Set(selectedFilters.map((item) => item.filterid)),
        ];
        dispatch(changeMasterdata(weekMasterid, "", currentWeek, currentYear));
      }
    } else {
      selectedFilterIDs = [
        ...new Set(
          selectedFilters
            .filter(
              (item) =>
                item.apiname !== "workweek" && item.apiname !== "workmonth"
            )
            .map((item) => item.filterid)
        ),
      ];
    ...
    // dispatch(removeAdvancedFilter(allFilterIDs));
    dispatch(resetCheckedValues(filterIDs));
    // clearFiltersDepandsonHorizontalGroupby
