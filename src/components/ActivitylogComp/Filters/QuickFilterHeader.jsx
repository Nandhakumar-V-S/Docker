...
import { ArcDateTimePicker } from "@/components/arccomponents/ui-components/ArcDateTimePicker/ArcDateTimePicker";
...
const QuickFilterHeader = ({
  ...
  const [startDateTime, setnewStartDateTime] = useState();
  const HandleDateChange = (date, id, api_name, setName) => {
  ...
  const HandleDateTimeChange = (datetime, id, api_name) => {
    const name = "Plan datetime";
    console.log({ datetime, id, api_name });
    
    // Format for API/database (full ISO string)
    const formattedDateTime = format(new Date(datetime), "MM/dd/yyyy HH:mm:ss");
    
    // Format for display (more readable)
    const tempDateTime = format(new Date(datetime), "EEE, do MMM, yyyy HH:mm a");
    console.log(tempDateTime);
    
    const selectedValue = [formattedDateTime];
    
    // Set the new start datetime
    setnewStartDateTime(datetime);
    
    // Dispatch actions to add filter and set datepicker
    dispatch(
      addSelectedFilter({
        id,
        selectedValue,
    ...
    dispatch(setDatepicker(id, api_name, name, tempDateTime, formattedDateTime));
  };
    ...
    if (quickFilterData) {
      return quickFilterData
        ...
        .map((filter) => {
            ...
            return ["datepicker"].includes(filter.filtercontroltype) ? (
              ...
              <>
                ...
                >
                  {selectedValueLength ? (
                    <button
                      className={`${activeFilterId === filter.id && "active"} ${IsFilter && filter.name && "is-filter"
          ...
          else if(filter.filtercategory!=="additional" && filter.filtercontroltype==="time"){
            return (
              <>
                <ArcDateFilter
                  startDate={startDate}
