...
import {
  ...
  verticalGroupbyOptionsInfo,
} from "@/redux/Activitylog/selector";

import {
  createGroupByData,
  updateGroupByDataDir,
  setVerticalGroupbyDir,
  createVerticalGroupbyData,
} from "@/redux/Activitylog/actions";
import VerticalGroupby from "@/components/arccomponents/VerticalGroupBy/VerticalGroupby";
...
export const GroupByFilter = ({
  ...
  }, [groupByData]); //GroupFilterPopup

  useEffect(() => {
    if (lookUpDetails?.length > 0) {
      const mappedItems = groupByData.filter(
        (item) => item.defaultvalue !== "" && item.type === "Horizontal"
      ...
      const updatedSelectedValues = mappedItems.map((item) => {
        ...
        return {
          ...
          value: item.defaultvalue, // Use defaultvalue as value
  ...
  }, [groupByData, lookUpDetails]); //GroupFilterPopup
  ...
  const handleSelectHorizontal = (selectedOption) => {
    handleHorizontalDirectionChange("asc");
  ...
  const verticalGroupbyColumns = useSelector(verticalGroupbySelectedInfo);
  const columnMasterData = useSelector(verticalGroupbyOptionsInfo);
  const verticalgroupbyOptions = columnMasterData;
  const [sortingShow, setSortingShow] = useState([]);
        ...
        <div className="footer-div">
          ...
          <button
            onClick={() => {
              ...
              dispatch(updateGroupByDataDir(HorizontalDir));
              const verticalGroupbyDatas = {
                columns: selectedColumnValue,
                // sorting: sortingShow,
              };
              dispatch(createVerticalGroupbyData(verticalGroupbyDatas));
            }}
          >
            Apply
          </button>
...
const GroupByDropDown = ({
  ...
  setSelectedValue,
  GroupFilterPopup,
  handleHorizontalDirectionChange,
}) => {
  // const [selectedValue, setSelectedValue] = useState(null);
  const [options, setOptions] = useState([]);

  const createdGroupByData = useSelector(createGroupByDataInfo);
  const groupByData = useSelector(newgroupbydataInfo);
  ...
  useEffect(() => {
    ...
    // If there is a default option and createdGroupByData is empty, set it as the first option
    if (defaultOption && createdGroupByData.length === 0) {
      if (!newOptions.some((option) => option.value === defaultOption.value)) {
        setOptions([defaultOption, ...newOptions]);
        setSelectedValue(defaultOption);
      } else {
        setOptions(newOptions);
      }
    } else if (createdGroupByData.length === 0) {
      setOptions(newOptions);
      setSelectedValue(null);
    }

    // Process createdGroupByData
    const setOption = createdGroupByData?.map((item) => ({
      value: item.id,
      label: item.name,
      type: item.type,
      masterid: item.masterid,
    ...
    if (setOption.length > 0) {
      // Remove duplicates before setting options
      const allOptions = [...newOptions, ...setOption].filter(
        (option, index, self) =>
          index === self.findIndex((o) => o.value === option.value)
      );

      setOptions(allOptions);
      setSelectedValue(setOption[0]);
    }
  }, [groupByData, type]); //GroupFilterPopup
    ...
    <div className="input-control">
      <div className="select-div">
        <Select
          ...
          isClearable
          placeholder="Select Horizontal Group By"
        />
      </div>
    </div>
  );
