...
import { createGroupByData } from "@/redux/TaskInfo/actions";

export const GroupByFilter = ({ GroupFilterPopup, setGroupFilterPopup, GroupFilterButtonRef }) => {
   ...
   useEffect(() => {
      const handleOutsideClick = (event) => {
         const buttonClicked =
            GroupFilterButtonRef.current && GroupFilterButtonRef.current.contains(event.target);
   ...
   useEffect(() => {
      if (lookUpDetails?.length > 0) {
         ...
         const updatedSelectedValues = mappedItems.map((item) => {
            ...
            const lookupDetail = lookUpDetails?.find((detail) => detail.masterid === item.masterid);
   ...
   const handleHorizontalDirectionChange = (direction) => {
      ...
      const horizontalIndex = selectedValues.findIndex((item) => item.type === "Horizontal");
   ...
   const handleVerticalDirectionChange = (direction) => {
      ...
      // Find the index of the object with type "Horizontal" in selectedValues
      const verticalIndex = selectedValues.findIndex((item) => item.type === "Vertical");
   ...
   const handleSelectVertical = (selectedOption) => {
      ...
      const existingIndex = selectedValues.findIndex((item) => item.type === selectedOption.type);
   ...
   return (
      <>
         <div className={`custom-filtered-div group-by-filter ${GroupFilterPopup && "active"}`}>
...
const GroupByDropDown = ({ type, onSelect, selectedValue, setSelectedValue, GroupFilterPopup }) => {
   ...
   }, [groupByData, type, createdGroupByData]); //GroupFilterPopup
