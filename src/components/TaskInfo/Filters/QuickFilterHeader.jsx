...
const QuickFilterHeader = ({
   ...
   // const { selectedFilterItem, setselectedFilterItem } = useContext(
   ...
   // console.log(setSelectedDropdownValue);
   useEffect(() => {
   ...
   useEffect(() => {
      ...
      if (quickFilterData && masterData) {
         quickFilterData
            .filter(
               (filter) => filter.isquickfilter && filter.name !== null && filter.masterid !== null
            )
            .forEach((filter) => {
               if (filter.masterid) {
                  ...
                  if (matchingLookup) {
                     ...
                     if (newMatchingLookup) {
                        ...
                        //   matchingLookup?.mastervalues &&
                        //   matchingLookup?.mastervalues
                        //     .filter((value) => defaultValues.includes(value.optionid))
                        //     .map((value) => value.optionid);
                        const defaultOptionIds = Array.isArray(matchingLookup?.mastervalues)
                           ? matchingLookup.mastervalues
                                .filter((value) => defaultValues.includes(value.optionid))
                                .map((value) => value.optionid)
                           : [];
                        ...
                        const updatedMatchingLookup = {
                           ...
                           defaultvalue:
                              defaultOptionIds.length === 1 ? defaultOptionIds[0] : defaultOptionIds
                        };
   ...
   useEffect(() => {
      if (quickFilterData && quickFilterData.length > 0 && masterData && masterData.length > 0) {
         var hasChecked;
         ...
         quickFilterData.forEach((item) => {
            if (
               ...
               (item.filtercontroltype === "groupdropdown" &&
                  item.filtercategory !== "additional" &&
                  item.masterid !== "")
            ) {
               const lookupDetail = masterData.find((detail) => detail.masterid === item.masterid);
               console.log(lookupDetail);
               if (lookupDetail && item.defaultvalue !== null && item.defaultvalue !== "") {
                  const defaultValues = item.defaultvalue.split(",").map((value) => value.trim());
                  ...
                  defaultOptions.forEach((defaultOption) => {
                     ...
                     if (existingOptionIndex !== -1) {
                        updatedSelectedOptions[existingOptionIndex].selectedValue = Array.from(
                           new Set([
                     ...
                     console.log(updatedSelectedOptions);
                     const matchedLookupItem = matchedLookup && matchedLookup[item.masterid];
                     ...
                     if (!hasChecked) {
                        dispatch(setMultiselectChecked(item.masterid, selectValues, true));
                     }
                  });
               }
            } else if (
            ...
            quickFilterData.forEach((item) => {
               if (
                  ...
                  if (lookupDetail && item.defaultvalue !== null && item.defaultvalue !== "") {
                     ...
                     if (defaultOption) {
                        dispatch(setDropdownChecked(item.masterid, defaultOption.optionid, true));
                     }
                  }
               }
            });
   ...
   useEffect(() => {
      if (selectedFilters.length > 0) {
         ...
         const newOptions = updatedOptions.filter(
            (option) => !selectedOptions.some((existing) => existing.id === option.id)
         );
   ...
   const renderFilterButtons = () => {
      if (quickFilterData) {
         return quickFilterData
            .filter((filter) => filter.ismapped && filter.name !== null && filter.masterid !== null)
            .map((filter) => {
               if (
                  //filter.ismapped &&
                  ...
                  const filterOptions = matchedLookup[filter.masterid].mastervalues || [];
                  // ~ groupdropdown count
                  ...
                  ) : ["dropdown", "multiselect"].includes(filter.filtercontroltype) ? (
                     // <>
                     ...
                     <>
                        ...
                        >
                           {selectedValueLength ? (
                              ...
                              <button className={`${activeFilterId === filter.id && "active"}`}>
                                 {Tableloading ? (
                                    <LoadingBtn />
                                 ) : (
                                    <>
                  ...
                  let selectedValueLength = textFilter && textFilter[filter.id]?.value ? 1 : 0;
                  return (
                     <OverlayTrigger
                        key={filter.id}
                        trigger='click'
                        ...
                        onToggle={() => togglePopover(filter.id)} // Toggle popover visibility
                        overlay={
                           <Popover id={`popover-positioned-auto`} className='custom-multi-select'>
                              <Popover.Body>
                                 <div className='content'>
                                    <div className='input-control'>
                                       <div className='header'>
   ...
   const renderFilterOptions = (filter) => {
      if (matchedLookup[filter.masterid]) {
         ...
         if (filter.filtercontroltype === "dropdown") {
            const sortedValues = SortingWithoutDirection(mastervalues, "optionvalue");
               ...
               <>
                  {sortedValues && (
                     <Select
                        ...
                        onChange={(selectedOption) => handleOptionChange(filter, selectedOption)}
                     ...
                     {mastervalues && (
                        <div className='multiselect'>
                           ...
                           {sortedValues.filter((option) =>
                              option.optionvalue?.toLowerCase().includes(searchTerm.toLowerCase())
                           ).length === 0 ? (
         ...
         return (
            <>
               <input
                  ...
                  type='text'
                  defaultValue={isFiltersCleared ? "" : textValues[filter.id]?.value || ""}
                  onChange={(e) => {
                     handleTextChange(filter, e.target.value);
                  }}
   ...
   const handleSelectAll = (event, filter, mastervalues) => {
      const isChecked = event.target.checked;
      const { id, api_name, filtercontroltype, filtercategory, masterid } = filter;
      const allOptionIds = mastervalues.map((option) => option.optionid);
   ...
   const handleCheckboxTagfilter = (mastervalue, filter) => {
      ...
      setselectedFilterItem((prevSelectedItem) => {
         ...
         if (isSelected) {
            return prevSelectedItem
               ...
               .map((item) => ({
                  ...item,
                  isChecked: item.optionid !== selectedOptionId ? item.isChecked : false
               }));
         } else {
            return [
               ...prevSelectedItem,
      ...
      const updateSelectedOption = (option) => {
         const selectedOptionIndex = option.selectedValue.indexOf(selectedOptionId);
         if (selectedOptionIndex !== -1) {
            return {
               ...option,
               selectedValue: option.selectedValue.filter((value) => value !== selectedOptionId)
            };
         } else {
            return {
               ...option,
      ...
      if (APPLY_BUTTON_ENABLED === "false") {
         const selectedOption = updatedSelectedOptions.find((option) => option.id === id);
         if (selectedOption && selectedOption.selectedValue) {
            ...
            dispatch(setMultiselectChecked(masterid, selectedOption.selectedValue, true));
            setActiveFilterId(null);
         }
      }
   ...
   const handleTextSubmit = (id, api_name, controltype, category, name) => {
      let selectedValue = textValues && textValues[id] ? textValues[id].value : null;
   ...
   const handleSelectSubmit = (filterId, optionId, masterid, screenfieldid, controltype) => {
   ...
   const handleMultiSubmit = (
      ...
      const selectedOption = selectedOptions.find((option) => option.filterId === filterId);
      if (selectedOption && selectedOption.selectedValue) {
         const Value = selectedOption.selectedValue;
         const selectedValue = Array.isArray(Value) ? Value : [Value];
   ...
   const handleRemoveFilter = (e, id, controltype) => {
      ...
      setSelectedOptions((prevItems) => prevItems?.filter((item) => item.id !== id));
      setselectedFilterItem([]);
   ...
   const clearFilters = () => {
      ...
      const selectedFilterIDs = [
         ...new Set(
            selectedFilters
               .filter((item) => item.apiname !== "workweek" && item.apiname !== "workday")
               .map((item) => item.filterid)
         )
      ];
      console.log(selectedFilterIDs);
      // Collect unique filter IDs from AdvancedFilters
      const advancedFilterIDs = [...new Set(additionalFilters.map((item) => item.filterid))];
      console.log(advancedFilterIDs);
      // Merge both arrays of filter IDs and remove duplicates
      const allFilterIDs = [...new Set([...selectedFilterIDs, ...advancedFilterIDs])];
