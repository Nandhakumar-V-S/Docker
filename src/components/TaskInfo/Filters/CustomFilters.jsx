...
export const CustomFilters = ({
   ...
   const handleArcAlertPopup = () => {
      ...
      payloadMap.forEach((currentAttrId) => {
         ...
         CurrentAttribute.forEach((id) => {
            const matchedObject = Object.values(matchedLookup).find((obj) => obj.id === id);
            console.log(matchedObject);
            if (matchedObject) {
               const { id, api_name, masterid, filtercontroltype, filtercategory } = matchedObject;
               const value = [];
            ...
            if (textFilter != {}) {
               ...
               const matchedText = Object.values(textFilter)?.find((obj) => obj.id === id);
               console.log(matchedText);
               if (matchedText) {
                  const { id, api_name, filtercategory, filtercontroltype } = matchedText;
                  const value = "";
   ...
   const handleApplyButtonClick = () => {
      ...
      const selectedAttributes = [...selectedMappedAttributes, ...selectedUnmappedAttributes];
      console.log(selectedAttributes);
      setCurrentAttribute(selectedAttributes);
      const existingFilters = selectedMappedAttributes.filter((id) => payloadMap?.includes(id));
   ...
   useEffect(() => {
      if (filterAddData && filterAddData.length > 0 && masterData && masterData.length > 0) {
         console.log("it runs");
         console.log(filterAddData);
            ...
            if (filter.masterid) {
               ...
               if (matchingLookup) {
                  ...
                  if (newMatchingLookup) {
                     ...
                     if (filter.filtercategory !== "quick" && filter.defaultvalue !== null) {
                        defaultValues = filter.defaultvalue.split(",").map((value) => value.trim());
                     }
            ...
            if (
               ...
               if (lookupDetail && item.defaultvalue !== null && item.defaultvalue !== "") {
                  ...
                  defaultOptions.forEach((defaultOption) => {
                     ...
                     dispatch(setAdvMultiOptionChecked(item.masterid, selectValues, true));
                  });
               }
            } else if (
            ...
            filterAddData.forEach((item) => {
               if (
                  ...
                  if (lookupDetail && item.defaultvalue !== null && item.defaultvalue !== "") {
                     ...
                     if (defaultOption) {
                        dispatch(setAdvOptionChecked(item.masterid, defaultOption.optionid, true));
                     }
                  }
               }
            });
      ...
      if (advMatchedLookup) {
         setSelectedItems((prevSelectedItems) => {
            ...
            Object.values(advMatchedLookup).forEach((lookup) => {
               ...
               if (lookup.masterid && Array.isArray(lookup.mastervalues)) {
      ...
      if (additionalFilters.length > 0) {
         ...
         const updTxtFilter = selectedItems.filter((item) => {
            ...
            const newTextFilter = updatedTextFilter.filter(
               (filter) => filter.id !== item.id && filter.filtercontroltype === "textbox"
            );
         ...
         if (updTxtFilter.length === 0 && Object.keys(textValues).length === 0) {
            ...
            if (updatedTextFilter) {
               const resultObject = updatedTextFilter.reduce((accumulator, filter) => {
         ...
         if (filter.filtercontroltype === "dropdown") {
            ...
            const getDefaultSelectValue = (filterId, mastervalues) => {
               if (mastervalues && mastervalues.length > 0) {
                  const selectedOption = selectedOptions.find((option) => option.id === filterId);

                  if (selectedOption && selectedOption.selectedValue.length > 0) {
                     return selectedOption.selectedValue.map((value) => ({
                        value: value,
                        label:
                           mastervalues.find((option) => option.optionid === value)?.optionvalue ||
                           ""
                     }));
                  } else {
            ...
            const sortedValues = [...mastervalues].sort((a, b) => {
               const aValue = getValue(a.optionvalue);
               const bValue = getValue(b.optionvalue);
                                 ...
                                 <input
                                    ...
                                    checked={selectedOptions.some(
                                       (selectedOption) =>
                                          ...
                                          selectedOption.selectedValue.includes(option.optionid)
                                    )}
                                    onChange={() =>
         ...
         if (!filter.masterid) {
            return (
               <input
                  ...
                  type='text'
                  value={textValues[filter.id] !== undefined ? textValues[filter.id].value : ""}
   ...
   const handleOptionChange = (filter, selectedOption) => {
      setSelectedOptions((prevState) => {
         ...
         const existingFilterIndex = updatedState.findIndex((item) => item.id === id);
      ...
      setSelectedDropdown((prevState) => {
         ...
         // Check if the filterId already exists in selectedOptions
         const existingFilterIndex = updatedState.findIndex((item) => item.filterId === id);
   ...
   const handleRemoveItem = (filterId, controltype, filtercategory) => {
      ...
      setSelectedItems((prevItems) => prevItems.filter((item) => item.id !== filterId));
   ...
   const handleCustomRemoveItems = (selectedOptions, textValues) => {
      ...
      selectedOptions.forEach((option) => {
         ...
         if (Array.isArray(option.selectedValue) && option.selectedValue.length === 0) {
      ...
      if (emptyFilterIds.length > 0) {
         ...
         setSelectedItems((prevItems) => prevItems.filter((item) => item.filterid !== filterId));
                              ...
                              <ul>
                                 {Object.values(matchedLookup).map((filter) => {
                                    ...
                                    if (checkedValues?.length === 1) {
                                       displayText = <span>{checkedValues[0].optionvalue}</span>;
                                       ...
                                       displayText = checkedValues.slice(0, -1).map((value) => (
                                          <span>
                                             {value.optionvalue}
                                             {", "}
                                          </span>
                                       ...
                                       displayText.push(
                                          <>
                                             {" "}
                                             or{" "}
                                             <span>{checkedValues?.slice(-1)[0].optionvalue}</span>
                                          </>
                                       );
                                    }
                                    ...
                                    return (
                                       <li key={filter.id}>
                                          <p>
                                             <span className='filter-title'>{name}</span> is any of{" "}
                                             {displayText}
                                          </p>
                                       </li>
                                    );
                           ...
                           <>
                              <div className='content added-values no-filter-div'>
                                 ...
                                 <p className='no-filter'>
                                    Add filters to arrow down the <br /> contacts you want to see.
                                 </p>
                              </div>
                           </>
                        ) : (
