...
import { setHorizontalGroupBy } from "@/redux/TaskInfo/actions";
...
export const HorizontalGrouping = ({ data, masterid, optionid, Tableloading, id }) => {
   ...
   console.log(data[0]);
   useEffect(() => {
      if (data && data[0] && prevDataRef.current.length > 0 && data[0] !== prevDataRef.current) {
   ...
   const handleSelect = (eventKey, event) => {
      const selectedItem =
         data && data[0] && data[0].slice(4).find((item) => item.optionid === eventKey);
   ...
   const selectedItem = data?.[0]?.slice(5).find((item) => item.optionid === activeOptionId);
               ...
               <>
                  {!Tableloading && (
                     <DropdownButton
                        ...
                        className={`${
                           selectedValue.optionid === activeOptionId || selectedItem?.optionvalue
                              ? "active"
                              : ""
                        }`}
                     >
