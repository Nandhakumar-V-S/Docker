...
const initialState = {
  entities: [],
  entityid: "0A5A8FC9-DDA0-4BE0-B0D3-58A81288F8DF",
  listid: "488C89A7-F628-4815-A73B-842501158FF7",
  newentityid: "6A054A51-CAED-4860-9934-754D3EBBAC7D",
  newlistid: "974DEFA2-42E6-4F2D-9AA3-0ACEE958BBA4",
  planentityid: "F48EF545-9995-4F8F-857D-DDDA2BC063CC",
  planlistid: "0A533D6B-5FC9-425E-B327-814AA7554CF5",
  userid: "",
  newEntity: "",
  masterData: [],
  subMasterData: [],
...
const ExportHistoryReducer = (state = initialState, action) => {
  switch (action.type) {
    case types.EXPORTHISTORY_GET_USERNAME: {
    ...
    case types.EXPORTHISTORY_FETCH_EXPORTHISTORY_DATA_SUCCESS: {
    ...
    case types.EXPORTHISTORY_INCREMENT_COUNT:
      return {
        ...state,
        renderCount: state.renderCount + 1,
      };
    case types.EXPORTHISTORY_SET_DATASETID:
      return {
        ...state,
        savedDatasetid: action.payload,
      };
    case types.EXPORTHISTORY_SELECT_NEW_ENTITY: {
    ...
    case types.EXPORTHISTORY_FETCH_MASTER_DATA_SUCCESS:
    ...
    case types.EXPORTHISTORY_FETCH_SUB_MASTER_DATA_SUCCESS:
      const submasterData = action.payload?.result?.data?.lookupvalues;
      const subparsedMasterData = submasterData.map((item) => ({
        ...item,
        mastervalues: JSON.parse(item.mastervalues),
      }));
      console.log(subparsedMasterData);
      return {
        ...state,
        subMasterData: subparsedMasterData,
      };
    case types.EXPORTHISTORY_FETCH_LIST_FIELDS_SUCCESS:
      const listfields =
        action.payload &&
    ...
    case types.EXPORTHISTORY_FETCH_LIST_DATA_SUCCESS:
      return {
        ...state,
        listData: action.payload?.result,
      };
    case types.EXPORTHISTORY_SET_PAGINATION:
    ...
    case types.EXPORTHISTORY_SET_PAGE_SIZE:
      return {
        ...state,
        postData: {
          ...state.postData,
    ...
    case types.EXPORTHISTORY_FETCH_DATASET_SUCCESS: {
      ...
      columns?.forEach((datasetColumn) => {
        ...
        if (sortedItem) {
          ...
          console.log("sortedItem masterid:", masterid);
          console.log("datasetColumn masterid:", datasetColumn.masterid);
      ...
      const sortedColumn = DataForColumns?.sort((a, b) => a.seqno - b.seqno);
      ...
      const updatednewQuickFilterData = QuickFilterData?.map((filter) => {
        if (
          filter.id === "41c3afe9-5958-4f32-b09d-08a74233f776" &&
          filter.defaultvalall === ""
        ) {
          console.log(filter);
          ...
          if (matchedMasterData) {
            const matchingOption = matchedMasterData.mastervalues.find(
              (value) =>
                value.optionvalue.toLowerCase() === state.username.toLowerCase()
            );
      ...
      if (filters.length > 0) {
        filters
          ?.filter((filter) => filter.filterid == "ExportedOn")
          .forEach((filter) => {
            console.log(filter);
            if (filter.defaultvalue === "") {
              ...
              const selectedFiltersToAdd = filters
                .filter(
                  (filter) =>
                    (filter.filterid === "workday" ||
                      filter.filterid == "ExportedOn") &&
                    filter.defaultvalue !== null &&
                    filter.defaultvalue !== undefined &&
                    filter.defaultvalue == ""
                )
      ...
      if (groupby.length > 0) {
        const defaultGroup = groupby
          .filter((item) => item.api_name === "workday_Group")
          .map((item) => ({
            filterid: item.id,
            apiname: item.api_name,
            filtervalue: item.defaultvalue,
      ...
      if (groupby.length > 0) {
        if (groupby.some((item) => item.api_name === "workday_Group")) {
        ...
        if (groupby.some((item) => item.api_name === "workweek_Group")) {
          updatedHorizontalGroupBy = groupby
            .filter((item) => item.api_name === "workweek_Group")
            .map((item) => {
              const masterMatch = updatedMasterData?.find(
                (data) => data.masterid === item.masterid
              );
              console.log(masterMatch);
              let matchedObject;
              ...
              if (masterMatch) {
                if (item.defaultvalue === "") {
                  matchedObject = masterMatch.mastervalues?.find(
                    (valueItem) => valueItem.value === groupValue
                  );
                } else {
                  matchedObject = masterMatch.mastervalues?.find(
                    (valueItem) => valueItem.value === item.defaultvalue
                  );
                }
              }
              ...
              return {
                api_name: item.api_name,
                masterid: item.masterid,
                mastervalues: masterMatch ? masterMatch.mastervalues : null,
                value: matchedObject ? matchedObject.optionid : null,
                id: item.id,
                grpbyDate:
                  item.defaultvalue === "" ? groupValue : item.defaultvalue,
              };
    ...
    case types.EXPORTHISTORY_SET_ADV_MATCHED_LOOKUP:
    ...
    case types.EXPORTHISTORY_SET_MATCHED_LOOKUP:
      if (!action.payload || Object.keys(action.payload).length === 0) {
        console.log(action.payload);
        return {
          ...state,
    ...
    case types.EXPORTHISTORY_TOGGLE_COLUMN_ISMAPPING: {
    ...
    case types.EXPORTHISTORY_TOGGLE_FILTER_ISMAPPING: {
    ...
    case types.EXPORTHISTORY_CREATE_HOR_GROUPBYDATA: {
      ...
      if (payload && payload.length > 0) {
        ...
        console.log(updatedCreateGroupByData);
        if (
          updatedCreateGroupByData.length > 0 &&
          updatedCreateGroupByData[0].api_name === "workweek_Group"
        ) {
          updatedCreateGroupByData
            .filter((item) => item.api_name === "workweek_Group")
            .forEach((item) => {
              const { id, api_name, value, grpbyDate } = item;
          ...
          updatedHorizontalGroupBy = updatedCreateGroupByData
            .filter((item) => item.api_name === "workweek_Group")
            .map((item) => ({
              masterid: item.masterid,
              ...
              value: item.value,
              id: item.id,
              grpbyDate: item.grpbyDate,
            }));
        }
        if (
          updatedCreateGroupByData.length > 0 &&
          updatedCreateGroupByData[0].api_name === "workday_Group"
        ) {
          updatedCreateGroupByData
            .filter((item) => item.api_name === "workday_Group")
            .forEach((item) => {
    ...
    case types.EXPORTHISTORY_CHANGE_HORIZONTAL_GROUPBY: {
      const { data, id, optionid } = action.payload;
      console.log(id, optionid);
      console.log(data);
      const updatedPostData = { ...state.postData };
      // const updatedCreateGroupByData = [...state.createGroupByData];
      const changedhorizontalGroupBy = [...state.horizontalGroupBy];
      // console.log(updatedCreateGroupByData);

      const sortedItem = data[0].find((item) => item.optionid === optionid);
      console.log(sortedItem);
      // Update filterparams
      if (
        changedhorizontalGroupBy.length > 0 &&
        changedhorizontalGroupBy[0].api_name === "workweek_Group"
      ) {
        updatedPostData.filterparams = updatedPostData.filterparams.map(
          (filter) => {
            if (filter.filterid === id) {
              return {
                ...filter,
                filtervalue: sortedItem.value, // Assuming optionid is a string
              };
            }
            return filter;
          }
        );
        const horGroupByIndex = changedhorizontalGroupBy.findIndex(
          (group) => group.id === id
        );
        changedhorizontalGroupBy[horGroupByIndex] = {
          ...changedhorizontalGroupBy[horGroupByIndex],
          value: optionid.toString(), // Assuming optionid is a string
          grpbyDate: sortedItem.value,
        };
      }
      if (
        changedhorizontalGroupBy.length > 0 &&
        changedhorizontalGroupBy[0].api_name === "workday_Group"
      ) {
        updatedPostData.filterparams = updatedPostData.filterparams.map(
          (filter) => {
            if (filter.filterid === id) {
      ...
      return {
        ...state,
        postData: updatedPostData,
        horizontalGroupBy: changedhorizontalGroupBy,
      };
    }

    case types.EXPORTHISTORY_SELECTED_FILTER_VALUE: {
    ...
    case types.EXPORTHISTORY_ADD_ADDITIONAL_FILTER: {
      const { selectedOptions, textvalues } = action.payload;
      console.log(selectedOptions, textvalues, "textvalues");
      ...
      if (Array.isArray(selectedOptions)) {
        console.log(typeof selectedOptions, "selectedOptions");
        ...
        selectedOptions?.forEach((option) => {
          ...
          if (quickFilterIndex !== -1) {
            if (selectedOptions[0].controltype === "datepicker") {
              const existingSelectedFilterIndex =
                updatedSelectedFilters.findIndex(
                  (filter) => filter.filterid === selectedOptions.id
                );
              console.log(existingSelectedFilterIndex);
              if (existingSelectedFilterIndex !== -1) {
                // If the filter already exists, update filtervalue in both updatedFilterParams and updatedSelectedFilters
                ...
                if (
                  selectedOptions[0].selectedValue.length === 0 ||
                  selectedOptions[0].selectedValue[0] === "" ||
                  selectedOptions[0].selectedValue === ""
                ) {
                  let textFilterDatas = updatedFilterParams.filter(
                    (filter) => filter.filtercategory === "quick"
                  );
                  ...
                  updatedFilterParams = textFilterDatas.filter((filter) => {
                    console.log(filter);
                    return (
                      filter.filterid === selectedOptions[0].id &&
                      filter.filtercategory !== "quick"
                    );
                  });
                  console.log(updatedFilterParams);
                  ...
                  updatedFilterParams = updatedPostData.filterparams.map(
                    (filter) => {
                      if (
                        filter.filterid === selectedOptions.id &&
                        filter.filtercategory === category
                      ) {
                        return {
                          ...filter,
                          filtervalue: selectedOptions[0].selectedValue[0]
                            ? selectedOptions[0].selectedValue[0]
                            : selectedOptions[0].selectedValue,
                        };
                      }
                  ...
                  // Update selectedFilters
                  updatedSelectedFilters = updatedSelectedFilters.map(
                    (filter) => {
                      if (filter.filterid === selectedOptions[0].id) {
                        return {
                          ...filter,
                          filtervalue: selectedOptions[0].selectedValue[0]
                            ? selectedOptions[0].selectedValue[0]
                ...
                // If the filter does not exist, add it as a new object to both updatedFilterParams and updatedSelectedFilters
                console.log(selectedOptions[0].selectedValue);
                if (selectedOptions[0]?.selectedValue?.length > 0) {
                  const newFilter = {
                    filterid: selectedOptions[0].id,
                    apiname: selectedOptions[0].api_name,
                    filtervalue: selectedOptions[0].selectedValue[0],
                    filtercategory: selectedOptions[0].filterCategory,
                    controltype: selectedOptions[0].controltype,
                    // filtername: name,
                    condition: "OR",
                  };
                  updatedFilterParams = [...updatedFilterParams, newFilter];
      ...
      let StructingForAppendNewDataForUpdatedPostData = {
        apiname: selectedOptions[0]?.api_name,
        condition: "OR", // Your required condition
        controltype: selectedOptions[0]?.controltype,
        filtercategory: selectedOptions[0]?.filterCategory,
        filterid: selectedOptions[0]?.id,
        filtervalue: Array.isArray(selectedOptions[0]?.selectedValue)
          ? selectedOptions[0]?.selectedValue[0]
          : null,
      };

      function replaceAllButFirst(newFilter) {
        if (updatedPostData?.filterparams?.length > 0) {
          const firstObject = updatedPostData.filterparams[0];
          updatedPostData.filterparams = [firstObject, newFilter];
          console.log("After replacement:", updatedPostData.filterparams);
        } else {
          updatedPostData.filterparams = [newFilter];
          console.log(
            "Array was empty, added new filter:",
            updatedPostData.filterparams
          );
        }
      }
      replaceAllButFirst(StructingForAppendNewDataForUpdatedPostData);
      ...
      updatedPostData = {
        ...updatedPostData,
        filterparams: updatedPostData.filterparams,
        start: 0,
      };
    ...
    case types.EXPORTHISTORY_SET_DROPDOWN_CHECKED: {
    ...
    case types.EXPORTHISTORY_SET_MULTISELECT_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.EXPORTHISTORY_REMOVE_ALL_FILTER: {
      console.log(action.payload);
    ...
    case types.EXPORTHISTORY_REMOVE_QUICK_FILTER: {
    ...
    case types.EXPORTHISTORY_REMOVE_ADVANCED_FILTER: {
      console.log(action.payload);
    ...
    case types.EXPORTHISTORY_QUICK_TEXT_VALUE: {
      console.log(action.payload);
      return {
        ...state,
    ...
    case types.EXPORTHISTORY_RESET_QUICK_FILTER_VALUES: {
      let filterIds = [];
      console.log(action.payload);
    ...
    case types.EXPORTHISTORY_CREATE_NEW_DATASET:
      return {
        ...state,
        newDataset: action.payload,
      };
    case types.EXPORTHISTORY_EDIT_DATASET:
      console.log(action.payload);
      return {
        ...state,
        updateDataSet: action.payload,
      };
    case types.EXPORTHISTORY_SORTED_COLUMN: {
    ...
    case types.EXPORTHISTORY_GET_NEW_DATASET_INFO_SUCCESS: {
    ...
    case types.EXPORTHISTORY_DELETE_DATASET_DB: {
      console.log(action.payload);
    ...
    case types.EXPORTHISTORY_SET_ADV_DROPDOWN_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.EXPORTHISTORY_SET_ADV_MULTISELECT_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.EXPORTHISTORY_SET_NEW_LAYOUT:
      const updatedTotalColumn = [...state.totalColumns];
    ...
    case types.EXPORTHISTORY_SET_COLUMN_SEQ: {
      const updatedTotalColumn = [...state.totalColumns];
      console.log(action.payload);
    ...
    case types.EXPORTHISTORY_SET_DATEPICKER_VALUE: {
    ...
    case types.EXPORTHISTORY_SET_WEEKFILTER_VALUE: {
    ...
    case types.EXPORTHISTORY_RESET_DATE_FILTERS: {
      console.log("it runs");
      return initialState;
    }
    case types.EXPORTHISTORY_CHANGE_MASTER_DATA: {
      ...
      const changeGroupby = [...state.horizontalGroupBy];
      console.log(changeGroupby);
      let changedHorGroupby;

      if (changeGroupby.some((item) => item.api_name === "workweek_Group")) {
        changedHorGroupby = changeGroupby
          .filter((item) => item.api_name === "workweek_Group")
          .map((item) => {
            const masterMatch = changedMasterdata?.find(
              (data) => data.masterid === item.masterid
            );
            console.log(masterMatch);
            var matchedgrpObject;
            // const today = new Date();
            // const groupValue = formatDate(today);
            if (masterMatch) {
              matchedgrpObject = masterMatch.mastervalues?.find(
                (valueItem) => valueItem.optionid === item.value
              );
            }

            console.log(matchedgrpObject);

            return {
              ...item,
              mastervalues: masterMatch ? masterMatch.mastervalues : null,
              value: matchedgrpObject ? matchedgrpObject.optionid : null,
              grpbyDate: matchedgrpObject ? matchedgrpObject.value : null,
            };
          });
      }
      console.log(changedHorGroupby);
      const updatedPostData = { ...state.postData };
      let updatedFilterParams = updatedPostData.filterparams.map((filter) => {
        if (filter.apiname === "workweek_Group") {
          return {
            ...filter,
            filtervalue: changedHorGroupby
              ? changedHorGroupby[0].grpbyDate
              : null,
          };
        }
        return filter;
      ...
      return {
        ...
        horizontalGroupBy: changedHorGroupby,
      };
    }

    case types.EXPORTHISTORY_SET_LOADING: {
      return {
        ...state,
        loading: action.payload,
      };
    }
    case types.EXPORTHISTORY_SET_TRANSACTIONID: {
      console.log(action.payload);
      return {
        ...state,
    ...
    case types.EXPORTHISTORY_360_FETCH_LIST_FIELDS_SUCCESS:
      const newlistfields =
        action.payload &&
        action.payload?.result?.list[0]?.data?.config?.listfields;
      console.log(newlistfields);

      const newdatasetArray =
        action.payload && action.payload?.result?.list[0]?.data?.dataset;
      console.log(newdatasetArray);
      let newselectedDataset = newdatasetArray?.find(
        (dataset) => dataset.isselected
      )?.datasetid;

      if (!newselectedDataset) {
        const defaultDataset = newdatasetArray?.find(
          (dataset) => dataset.isdefault
        );
        newselectedDataset = defaultDataset?.datasetid;
      }

      console.log(newselectedDataset);
      return {
        ...state,
        newColumnMasterData: newlistfields,
        newdataSetID: newselectedDataset,
      };
    case types.EXPORTHISTORY_360_DATASET_SUCCESS: {
      console.log(action.payload);
      const { columns } = action.payload?.result;
      const columndata = [...state.newColumnMasterData];
      ...
      console.log(columnIdsSet);
      // Filter newColumnMasterData to find items not present in columns
      const unmatchedItems = columndata
        .filter((item) => !columnIdsSet.has(item.id))
        .map((item) => ({ ...item, ismapped: false }));
      console.log(unmatchedItems);
      ...
      columns?.forEach((datasetColumn) => {
        // Find matching item in sortedColumn array
        const sortedItem = columndata.find(
          (item) => item.id === datasetColumn.id
        );
        if (sortedItem) {
          // If match found, collect required properties
      ...
      const newtotalColumns = [...sortedColumn, ...sortedUnMappedColumns];
      console.log(newtotalColumns);
      return {
        ...state,
        newTotalcolumn: newtotalColumns,
      };
    }
    case types.EXPORTHISTORY_360_TASK_ROW_DATA: {
      console.log(action.payload);
      return {
        ...state,
        Taskrowdata: action.payload?.result,
      };
    }
    case types.EXPORTHISTORY_360_FETCH_SUB_LIST_FIELDS_SUCCESS: {
      const newlistfields =
        action.payload &&
        action.payload?.result?.list[0]?.data?.config?.listfields;
      console.log(newlistfields);
      ...
      return {
        ...state,
        planColumnMasterData: newlistfields,
        plandataSetID: newselectedDataset,
      };
    }
    case types.EXPORTHISTORY_360_SUB_DATASET_SUCCESS: {
      console.log(action.payload);
      const { columns } = action.payload?.result;
      const columndata = [...state.planColumnMasterData];
      ...
      return {
        ...state,
        plannewTotalcolumn: newtotalColumns,
      };
    }
    case types.EXPORTHISTORY_360_SUB_TASK_ROW_DATA: {
      console.log(action.payload);
      return {
        ...state,
        planTaskrowdata: action.payload?.result,
      };
    }
    case types.EXPORTHISTORY_EDIT_CELL: {
      const { transactionid, attributedatatype, listconfigid, apiname, value } =
        action.payload;
      const loggeduser = state.userid;
      return {
        ...state,
        editcell: {
          ...state.editcell,
    ...
    case types.EXPORTHISTORY_RESET_EDIT_CELL: {
      const resetEditcell = {
