...
const initialState = {
  ...
  att360entityid: "70C75211-155C-45DC-B155-002DBB5CCE91",
  att360listid: "8B50916F-41EC-4D7E-B362-6706D121522B",
  userid: "",
  newEntity: "",
  masterData: [],
  subMasterData: [],
...
const ExecutionReducer = (state = initialState, action) => {
  switch (action.type) {
    case types.EXECUTION_SET_CURRENTPAGE: {
    ...
    case types.EXECUTION_GET_USERNAME: {
    ...
    case types.EXECUTION_FETCH_EXECUTION_DATA_SUCCESS: {
    ...
    case types.EXECUTION_INCREMENT_COUNT:
      return {
        ...state,
        renderCount: state.renderCount + 1,
      };
    case types.EXECUTION_SET_DATASETID:
      return {
        ...state,
        savedDatasetid: action.payload,
      };
    case types.EXECUTION_SELECT_NEW_ENTITY: {
    ...
    case types.EXECUTION_FETCH_MASTER_DATA_SUCCESS:
    ...
    case types.EXECUTION_FETCH_SUB_MASTER_DATA_SUCCESS:
    ...
    case types.EXECUTION_FETCH_LIST_FIELDS_SUCCESS:
      const listfields =
        action.payload &&
    ...
    case types.EXECUTION_FETCH_LIST_DATA_SUCCESS:
    ...
    case types.EXECUTION_SET_PAGINATION:
    ...
    case types.EXECUTION_SET_PAGE_SIZE:
      return {
        ...state,
    ...
    case types.EXECUTION_FETCH_DATASET_SUCCESS: {
      console.log(action.payload);
      ...
      const updatednewQuickFilterData = QuickFilterData?.map((filter) => {
        if (
          ...
          console.log(matchedMasterData, "matchedMasterData");
      ...
      if (filters.length > 0) {
        filters
          ?.filter((filter) => filter.filterid === "workday")
          .forEach((filter) => {
    ...
    case types.EXECUTION_SET_ADV_MATCHED_LOOKUP:
    ...
    case types.EXECUTION_SET_MATCHED_LOOKUP:
      if (!action.payload || Object.keys(action.payload).length === 0) {
        console.log(action.payload);
        return {
          ...state,
    ...
    case types.EXECUTION_TOGGLE_COLUMN_ISMAPPING: {
    ...
    case types.EXECUTION_TOGGLE_FILTER_ISMAPPING: {
    ...
    case types.EXECUTION_CREATE_HOR_GROUPBYDATA: {
    ...
    case types.EXECUTION_CHANGE_HORIZONTAL_GROUPBY: {
      ...
      const sortedItem = data.find((item) => item.groupbyid === optionid);
      console.log(sortedItem);
      // Update filterparams
      if (
        ...
        updatedPostData.filterparams = updatedPostData.filterparams.map(
          (filter) => {
            if (filter.filterid === id) {
              return {
                ...filter,
                filtervalue: optionid === "All" ? optionid : sortedItem?.value, // Assuming optionid is a string
              };
            }
            return filter;
          }
        ...
        changedhorizontalGroupBy[horGroupByIndex] = {
          ...
          grpbyDate: optionid === "All" ? optionid : sortedItem?.value,
        };
      }
      if (
        changedhorizontalGroupBy.length > 0 &&
    ...
    case types.EXECUTION_SELECTED_FILTER_VALUE: {
    ...
    case types.EXECUTION_ADD_ADDITIONAL_FILTER: {
    ...
    case types.EXECUTION_SET_DROPDOWN_CHECKED: {
    ...
    case types.EXECUTION_SET_MULTISELECT_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.EXECUTION_REMOVE_ALL_FILTER: {
      console.log(action.payload);
      ...
      let updatedPostData = { ...state.postData };
      console.log(updatedPostData.filterparams);
    ...
    case types.EXECUTION_REMOVE_QUICK_FILTER: {
    ...
    case types.EXECUTION_REMOVE_ADVANCED_FILTER: {
      console.log(action.payload);
    ...
    case types.EXECUTION_QUICK_TEXT_VALUE: {
      console.log(action.payload);
      return {
        ...state,
    ...
    case types.EXECUTION_RESET_QUICK_FILTER_VALUES: {
      let filterIds = [];
      console.log(action.payload);
    ...
    case types.EXECUTION_CREATE_NEW_DATASET:
      return {
        ...state,
        newDataset: action.payload,
      };
    case types.EXECUTION_EDIT_DATASET:
      console.log(action.payload);
      return {
        ...state,
        updateDataSet: action.payload,
      };
    case types.EXECUTION_SORTED_COLUMN: {
    ...
    case types.EXECUTION_GET_NEW_DATASET_INFO_SUCCESS: {
    ...
    case types.EXECUTION_DELETE_DATASET_DB: {
      console.log(action.payload);
    ...
    case types.EXECUTION_SET_ADV_DROPDOWN_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.EXECUTION_SET_ADV_MULTISELECT_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.EXECUTION_SET_NEW_LAYOUT:
      const updatedTotalColumn = [...state.totalColumns];
    ...
    case types.EXECUTION_SET_COLUMN_SEQ: {
      const updatedTotalColumn = [...state.totalColumns];
      console.log(action.payload);
    ...
    case types.EXECUTION_SET_DATEPICKER_VALUE: {
    ...
    case types.EXECUTION_SET_WEEKFILTER_VALUE: {
    ...
    case types.EXECUTION_RESET_DATE_FILTERS: {
      console.log("it runs");
      return initialState;
    }
    case types.EXECUTION_CHANGE_MASTER_DATA: {
      const { masterid, week, selectedYear } = action.payload;

      var changedMasterdata = state.masterData?.map((masterItem) => {
        if (masterItem.masterid === masterid) {
          ...
          const newupdatedMastervalues = masterItem.mastervalues.map(
            (value, index) => {
              const date = new Date(startDate);
              date.setDate(startDate.getDate() + index);
              return { ...value, value: formatDate(date) };
          ...
          console.log(newupdatedMastervalues);
          // Return the updated masterItem
          return { ...masterItem, mastervalues: newupdatedMastervalues };
        }
        console.log(masterItem);
      ...
      if (changeGroupby.some((item) => item.api_name === "workday_Group")) {
        changedHorGroupby = changeGroupby
          .filter((item) => item.api_name === "workday_Group")
          .map((item) => {
            const masterMatch = changedMasterdata?.find(
    ...
    case types.EXECUTION_SET_LOADING: {
      return {
        ...state,
        loading: action.payload,
      };
    }
    case types.EXECUTION_SET_TRANSACTIONID: {
      console.log(action.payload);
      return {
        ...state,
    ...
    case types.EXECUTION_360_FETCH_LIST_FIELDS_SUCCESS:
      const newlistfields =
        action.payload &&
        action.payload?.result?.list[0]?.data?.config?.listfields;
    ...
    case types.EXECUTION_360_DATASET_SUCCESS: {
      console.log(action.payload);
      const { columns } = action.payload?.result;
    ...
    case types.EXECUTION_360_TASK_ROW_DATA: {
      console.log(action.payload);
      return {
        ...state,
    ...
    case types.EXECUTION_360_FETCH_SUB_LIST_FIELDS_SUCCESS: {
      const newlistfields =
        action.payload &&
        action.payload?.result?.list[0]?.data?.config?.listfields;
      console.log(newlistfields);
    ...
    case types.EXECUTION_360_SUB_DATASET_SUCCESS: {
      console.log(action.payload);
      const { columns } = action.payload?.result;
    ...
    case types.EXECUTION_360_SUB_TASK_ROW_DATA: {
      console.log(action.payload);
      return {
        ...state,
    ...
    case types.EXECUTION_EDIT_CELL: {
    ...
    case types.EXECUTION_RESET_EDIT_CELL: {
      const resetEditcell = {
        entityid: "F48EF545-9995-4F8F-857D-DDDA2BC063CC",
        transactionid: "",
        workitemid: null,
