...
const initialState = {
  ...
  listid: "0C68EE9D-2E21-4E25-85B2-4AFA61197F1E",
  userid: "",
  newEntity: "",
  masterData: [],
  list: [],
...
const ResourceVsMonthWeekReportReducer = (state = initialState, action) => {
  switch (action.type) {
    case types.RESOURCEVSMONTHWEEKREPORT_SET_CURRENTPAGE: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_GET_USERNAME: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_FETCH_ENTITY_DATA_SUCCESS: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_INCREMENT_COUNT:
      return {
        ...state,
        renderCount: state.renderCount + 1,
      };
    case types.RESOURCEVSMONTHWEEKREPORT_SET_DATASETID:
      return {
        ...state,
        savedDatasetid: action.payload,
      };
    case types.RESOURCEVSMONTHWEEKREPORT_SELECT_NEW_ENTITY: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_FETCH_MASTER_DATA_SUCCESS:
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_FETCH_LIST_FIELDS_SUCCESS:
      const listfields =
        action.payload &&
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_FETCH_LIST_DATA_SUCCESS:
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SET_PAGINATION:
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SET_PAGE_SIZE:
      return {
        ...state,
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_FETCH_DATASET_SUCCESS: {
      console.log(action.payload);
      ...
      if (filters?.length > 0) {
        ...
        console.log(selectedFiltersToAdd);
      ...
      if (filters?.length > 0) {
          ...
          var updatedMasterData = state.masterData?.map((masterItem) => {
            ...
            if (monthFilterItem && monthFilterItem.defaultvalue !== null) {
              ...
              if (monthFilterItem.defaultvalue === "") {
                ...
                const currentWeek = getWeek(today);
                const currentYear = today.getFullYear();
                const firstWeekNumber = getFirstWeekNumberOfCurrentMonth();
                firstweek = firstWeekNumber;
                crntyear = currentYear;
                console.log(firstWeekNumber);
                const pstGrpby = `${firstWeekNumber}$${currentYear}`;

                // startDate = getDateOfWeek(currentWeek, currentYear);
              } else {
                const [week, year] = monthFilterItem.defaultvalue.split("$");
                startDate = getDateOfWeek(parseInt(week), parseInt(year));
              }
              console.log(startDate);
              var updatedMastervalues = masterItem.mastervalues.map(
                (value, index) => {
                  // const date = new Date(startDate);
                  // date.setDate(startDate.getDate() + index);
                  const weekvalue = `${firstweek + index}$${crntyear}`;
                  return { ...value, value: weekvalue };
                }
              );
              console.log(updatedMastervalues);
              return { ...masterItem, mastervalues: updatedMastervalues };
        ...
        const groupValue = currentWeek + "$" + currentYear;
        // const today = new Date();
        // const groupValue = formatDate(today);
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SET_ADV_MATCHED_LOOKUP:
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SET_MATCHED_LOOKUP:
      if (!action.payload || Object.keys(action.payload).length === 0) {
        console.log(action.payload);
        return {
          ...state,
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_TOGGLE_COLUMN_ISMAPPING: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_TOGGLE_FILTER_ISMAPPING: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_CREATE_HOR_GROUPBYDATA: {
      ...
      if (payload && payload.length > 0) {
        ...
        if (
          ...
          updatedCreateGroupByData
            .filter((item) => item.api_name === "workweek_Group")
            .forEach((item) => {
              const { id, api_name, value, grpbyweek } = item;
              console.log(grpbyweek);
              const filterIndex = updatedPostData.filterparams.findIndex(
                (filter) => filter.filterid === id
              );
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_CHANGE_HORIZONTAL_GROUPBY: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SELECTED_FILTER_VALUE: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_ADD_ADDITIONAL_FILTER: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SET_DROPDOWN_CHECKED: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SET_MULTISELECT_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_REMOVE_ALL_FILTER: {
      console.log(action.payload);
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_REMOVE_QUICK_FILTER: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_REMOVE_ADVANCED_FILTER: {
      console.log(action.payload);
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_QUICK_TEXT_VALUE: {
      console.log(action.payload);
      return {
        ...state,
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_RESET_QUICK_FILTER_VALUES: {
      let filterIds = [];
      console.log(action.payload);
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_CREATE_NEW_DATASET:
      return {
        ...state,
        newDataset: action.payload,
      };
    case types.RESOURCEVSMONTHWEEKREPORT_EDIT_DATASET:
      console.log(action.payload);
      return {
        ...state,
        updateDataSet: action.payload,
      };
    case types.RESOURCEVSMONTHWEEKREPORT_SORTED_COLUMN: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_GET_NEW_DATASET_INFO_SUCCESS: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_DELETE_DATASET_DB: {
      console.log(action.payload);
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SET_ADV_DROPDOWN_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SET_ADV_MULTISELECT_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SET_NEW_LAYOUT:
      const updatedTotalColumn = [...state.totalColumns];
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SET_COLUMN_SEQ: {
      const updatedTotalColumn = [...state.totalColumns];
      console.log(action.payload);
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SET_DATEPICKER_VALUE: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SET_WEEKFILTER_VALUE: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SET_MONTHFILTER_VALUE: {
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_RESET_DATE_FILTERS: {
      console.log("it runs");
      return initialState;
    }
    case types.RESOURCEVSMONTHWEEKREPORT_CHANGE_MASTER_DATA: {
      const { masterid, week, selectedYear } = action.payload;
      // const changeGroupby = [...state.horizontalGroupBy];

      const changeGroupby = [...state.horizontalGroupBy];

      var changedMasterdata;
      if (changeGroupby.some((item) => item.api_name === "workday_Group")) {
        changedMasterdata = state.masterData?.map((masterItem) => {
        ...
        changeGroupby.some((item) => item.api_name === "workweek_Group")
      ) {
        changedMasterdata = state.masterData?.map((masterItem) => {
          if (masterItem.masterid === masterid) {
            console.log(state.masterData);
            const firstWeekNumber = gtfrstwkNum(
              Number(selectedYear),
              Number(week)
            );
            const newupdatedMastervalues = masterItem.mastervalues.map(
        ...
        changedHorGroupby = changeGroupby
          ...
          .map((item) => {
            ...
            return {
              ...
              mastervalues: masterMatch ? masterMatch.mastervalues : null,
              value: matchedgrpObject ? matchedgrpObject.optionid : null,
              grpbyweek: matchedgrpObject ? matchedgrpObject.value : null,
            };
          });
      }
      ...
      let updatedFilterParams = updatedPostData.filterparams.map((filter) => {
        if (filter.apiname === "workday_Group") {
          return {
            ...filter,
    ...
    case types.RESOURCEVSMONTHWEEKREPORT_SET_LOADING: {
      return {
        ...state,
...
export default ResourceVsMonthWeekReportReducer;
...
function getFirstWeekNumberOfCurrentMonth() {
  const today = new Date();
  const firstDayOfMonth = getFirstDayOfMonth(today);
  return getWeekNumberOfYear(firstDayOfMonth);
}
