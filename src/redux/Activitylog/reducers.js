...
const ActivitylogReducer = (state = initialState, action) => {
  switch (action.type) {
    case types.ACTIVITYLOG_SETMATRIXROWCOLUMN: {
      // const { rowData, columnData } = action.payload;
    ...
    case types.ACTIVITYLOG_GET_USERNAME: {
    ...
    case types.ACTIVITYLOG_CREATE_VER_GROUPBYDATA: {
      console.log(action.payload);
      const { columns, sorting } = action.payload;
      const { direction } = state.verticalGroupbyDirection;
      let updatedPostData = { ...state.postData };
      let orderBy = "";
      // let direction = "";
      // if (sorting.length > 0) {
      //   const sortable = sorting[0];
      ...
      // console.log(columns);
      // if (columns?.length > 0) {
      //   const lastObject = columns[columns.length - 1];
      //   console.log(lastObject);
      //   updatedPostData = {
      //     ...updatedPostData,
      //     orderby: lastObject?.binding,
      //     orderbydir: direction,
      //   };
      // }
      ...
      return {
        ...state,
        verticalGroupbyColumns: action.payload,
        // postData: updatedPostData,
      };
    }
    case types.ACTIVITYLOG_GET_USERMAIL: {
    ...
    case types.ACTIVITYLOG_FETCH_ENTITY_DATA_SUCCESS: {
    ...
    case types.ACTIVITYLOG_INCREMENT_COUNT:
      return {
        ...state,
        renderCount: state.renderCount + 1,
      };
    case types.ACTIVITYLOG_SET_DATASETID:
      return {
        ...state,
        savedDatasetid: action.payload,
      };
    case types.ACTIVITYLOG_SELECT_NEW_ENTITY: {
    ...
    case types.ACTIVITYLOG_FETCH_MASTER_DATA_SUCCESS:
    ...
    case types.ACTIVITYLOG_FETCH_LIST_FIELDS_SUCCESS:
      const listfields =
        action.payload &&
      ...
      // const horizontalGroupby = groupbyfields.filter(
      //   (item) => item.type?.toLowerCase() === "horizontal"
      // );
      const verticalGroupbyOptions = GetVerticalGroupbyOptions(
        groupbyfields || [],
        listfields || []
      );
      return {
        ...
        verticalGroupbyOptions: verticalGroupbyOptions || [],
        dataSetID: selectedDataset,
        dataSetList: datasetArray,
        newDatasetID: "",
      };
    case types.ACTIVITYLOG_FETCH_LIST_DATA_SUCCESS:
    ...
    case types.ACTIVITYLOG_SET_PAGINATION:
    ...
    case types.ACTIVITYLOG_SET_PAGE_SIZE:
      return {
        ...state,
    ...
    case types.ACTIVITYLOG_FETCH_DATASET_SUCCESS: {
      console.log(action.payload);
      ...
      const { listsortdir } = action.payload?.result;
      const { listsortfield } = action.payload?.result;
      ...
      const updatedPostData = {
        ...
        start: 0,
        skip: 20,
        orderby: listsortfield || "createdon",
        orderbydir: listsortdir || "Desc",
        filterparams: [],
      };
      ...
      const updatednewQuickFilterData = QuickFilterData?.map((filter) => {
        if (filter.name === "Resource" && filter.defaultvalall === "") {
          ...
          let logginInUserId = window.sessionStorage.getItem("Globalid");
          if (matchedMasterData) {
            const matchingOption = matchedMasterData.mastervalues.find(
            ...
            const matchingOption1 = matchedMasterData.mastervalues.find(
              (value) => value.optionid == logginInUserId
            );
            ...
            userobj.push(matchingOption);
            loginUserIDobj.push(matchingOption1);
          }
          // userobj.push(matchingOption);
          // userobj.push(matchingOption);
        }
      ...
      //vertical groupby
      let selectedVerticalGroupby;
      let verGrpBy;
      if (groupby.length > 0) {
        const defaultVerticalGroupby = groupby.filter(
          (item) => item.type === "vertical"
        );
        if (defaultVerticalGroupby.length > 0) {
          // const defaultApply = state.groupbyMaster
          //   ?.filter(
          //     (item) =>{
          //       defaultVerticalGroupby.forEach((data)=>{
          //         if(item.type==="vertical" && item.id===data.id)
          //       })
          //     }
          //       // item.type === "vertical" &&
          //       // item.id === defaultVerticalGroupby[0].id
          //   )
          //   .map((item) => ({
          //     value: item.id,
          //     direction: defaultVerticalGroupby[0].direction,
          //     api_name: defaultVerticalGroupby[0].api_name,
          //     type: defaultVerticalGroupby[0].type,
          //     seqno: defaultVerticalGroupby[0]?.seqno,
          //     label: item.name,
          //   }));
          const defaultApply = state.groupMasterData
            ?.filter(
              (item) =>
                item.type === "vertical" &&
                defaultVerticalGroupby.some((data) => data.id === item.id)
            )
            .map((item) => {
              // Find the matching groupby record for this item
              const matchingGroupby = defaultVerticalGroupby.find(
                (data) => data.id === item.id
              );

              return {
                value: item.id,
                direction: matchingGroupby.direction,
                api_name: matchingGroupby.api_name,
                type: matchingGroupby.type,
                seqno: item?.seq,
                label: item.name,
              };
            });
          console.log(defaultApply);
          console.log(state.listHeaders);
          selectedVerticalGroupby = defaultApply;
          // const selectedGrpValue = GetVerticalGroupbyOptions(defaultApply);
          if (defaultApply?.length > 0) {
            const listDetails = { ...state.columnMasterData };
            const verticalGroupbyOptions = GetVerticalGroupbyOptions(
              defaultApply || [],
              state.columnMasterData || []
            );
            if (verticalGroupbyOptions?.length > 0) {
              const bindingGroupby = verticalGroupbyOptions.map((item) => ({
                ...item,
                binding: item.displayname?.toLowerCase(),
              }));
              verGrpBy = bindingGroupby;
            }

            console.log(verticalGroupbyOptions);
          }
          console.log(verGrpBy);
        }
        console.log(verGrpBy);
        console.log(state.verticalGroupbyOptions);
        console.log(state.groupbyMaster);
        console.log(state.listHeaders);

        // console.log(verticalGroupbyOptions)
        // console.log(defaultVerticalGroupby)
      }
      //vertical groupby
      if (updatedMasterData !== undefined) {
        console.log(loginUserIDobj, "loginUserIDobj");
        if (verGrpBy !== undefined) {
          return {
            ...state,
            ...
            userobj: userobj,
            loginUserIDobj: loginUserIDobj,
            verticalGroupbyColumns: {
              ...state.verticalGroupbyColumns,
              columns: verGrpBy || [],
            },
          };
        } else {
    ...
    case types.ACTIVITYLOG_SET_ADV_MATCHED_LOOKUP:
    ...
    case types.ACTIVITYLOG_SET_MATCHED_LOOKUP:
      if (!action.payload || Object.keys(action.payload).length === 0) {
        console.log(action.payload);
        return {
          ...state,
    ...
    case types.ACTIVITYLOG_TOGGLE_COLUMN_ISMAPPING: {
    ...
    case types.ACTIVITYLOG_TOGGLE_FILTER_ISMAPPING: {
    ...
    case types.ACTIVITYLOG_CREATE_HOR_GROUPBYDATA_DIRECTION: {
      const { payload } = action;
      console.log(payload);
      return {
        ...state,
        horizontalGroupByDir: payload,
      };
    }
    case types.ACTIVITYLOG_CREATE_HOR_GROUPBYDATA: {
      const { payload } = action;
      console.log(payload);
      ...
      if (payload && payload.length > 0) {
        ...
        updatedHorizontalGroupBy = updatedCreateGroupByData.map((item) => {
          const sortedMastervalues = SortingWithDirection(
            item.mastervalues,
            item.dir,
            "optionvalue"
          );
    ...
    case types.ACTIVITYLOG_CHANGE_HORIZONTAL_GROUPBY: {
    ...
    case types.ACTIVITYLOG_SELECTED_FILTER_VALUE: {
    ...
    case types.ACTIVITYLOG_ADD_ADDITIONAL_FILTER: {
    ...
    case types.ACTIVITYLOG_SET_DROPDOWN_CHECKED: {
    ...
    case types.ACTIVITYLOG_SET_MULTISELECT_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.ACTIVITYLOG_REMOVE_ALL_FILTER: {
      console.log(action.payload);
    ...
    case types.ACTIVITYLOG_REMOVE_QUICK_FILTER: {
    ...
    case types.ACTIVITYLOG_REMOVE_ADVANCED_FILTER: {
      console.log(action.payload);
    ...
    case types.ACTIVITYLOG_QUICK_TEXT_VALUE: {
      console.log(action.payload);
      return {
        ...state,
    ...
    case types.ACTIVITYLOG_RESET_QUICK_FILTER_VALUES: {
      let filterIds = [];
      console.log(action.payload);
    ...
    case types.ACTIVITYLOG_CREATE_NEW_DATASET:
      return {
        ...state,
        newDataset: action.payload,
      };
    case types.ACTIVITYLOG_EDIT_DATASET:
      console.log(action.payload);
      return {
        ...state,
        updateDataSet: action.payload,
      };
    case types.ACTIVITYLOG_SORTED_COLUMN: {
    ...
    case types.ACTIVITYLOG_DELETE_DATASET_DB: {
      console.log(action.payload);
    ...
    case types.ACTIVITYLOG_SET_ADV_DROPDOWN_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.ACTIVITYLOG_SET_ADV_MULTISELECT_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.ACTIVITYLOG_SET_NEW_LAYOUT:
      const updatedTotalColumn = [...state.totalColumns];
    ...
    case types.ACTIVITYLOG_SET_COLUMN_SEQ: {
      const updatedTotalColumn = [...state.totalColumns];
      console.log(action.payload);
    ...
    case types.ACTIVITYLOG_SET_DATEPICKER_VALUE: {
    ...
    case types.ACTIVITYLOG_SET_WEEKFILTER_VALUE: {
    ...
    case types.ACTIVITYLOG_RESET_DATE_FILTERS: {
      console.log("it runs");
      return initialState;
    }
    case types.ACTIVITYLOG_CHANGE_MASTER_DATA: {
    ...
    case types.ACTIVITYLOG_SET_LOADING: {
      return {
        ...state,
        loading: action.payload,
      };
    }
    case types.ACTIVITYLOG_EDIT_CELL: {
    ...
    case types.ACTIVITYLOG_SET_CURRENTPAGE: {
      console.log(action.payload);
      return {
        ...state,
    ...
    case types.ACTIVITYLOG_RESET_EDIT_CELL: {
      const resetEditcell = {
        entityid: "26B84156-CC30-416E-99D5-B37409B4D0BD",
        transactionid: null,
        workitemid: "",
    ...
    case types.ACTIVITYLOG_SETVERTICAL_GROUPBY_DIR: {
      console.log(action.payload);
      return {
        ...state,
        verticalGroupbyDirection: action.payload,
      };
    }
    default:
      return state;
