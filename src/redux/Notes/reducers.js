...
import { SortingWithDirection } from "@/utils/CommonFunctions";
import * as types from "./types";

const initialState = {
  entities: [],
  entityid: "4AFB7451-C0D6-4BC8-BAAA-51ED99F1A323",
  listid: "8323A251-DF08-493F-9906-DB137AAF6961",
  userid: "",
  email: "",
  newEntity: "",
  masterData: [],
...
const NotesReducer = (state = initialState, action) => {
  switch (action.type) {
    case types.NOTES_SET_CURRENTPAGE: {
    ...
    case types.NOTES_FETCH_ENTITY_DATA_SUCCESS: {
    ...
    case types.NOTES_INCREMENT_COUNT:
      return {
        ...state,
        renderCount: state.renderCount + 1,
      };
    case types.NOTES_SET_DATASETID:
      return {
        ...state,
        savedDatasetid: action.payload,
      };
    case types.NOTES_SELECT_NEW_ENTITY: {
    ...
    case types.NOTES_FETCH_MASTER_DATA_SUCCESS:
    ...
    case types.NOTES_FETCH_LIST_FIELDS_SUCCESS:
      const listfields =
        action.payload &&
    ...
    case types.NOTES_SET_PAGE_SIZE:
      return {
        ...state,
    ...
    case types.NOTES_FETCH_DATASET_SUCCESS: {
      console.log(action.payload);
    ...
    case types.NOTES_SET_ADV_MATCHED_LOOKUP:
    ...
    case types.NOTES_SET_MATCHED_LOOKUP:
      if (!action.payload || Object.keys(action.payload).length === 0) {
        console.log(action.payload);
        return {
          ...state,
    ...
    case types.NOTES_TOGGLE_COLUMN_ISMAPPING: {
    ...
    case types.NOTES_TOGGLE_FILTER_ISMAPPING: {
    ...
    case types.NOTES_CREATE_HOR_GROUPBYDATA: {
    ...
    case types.NOTES_CHANGE_HORIZONTAL_GROUPBY: {
    ...
    case types.NOTES_SELECTED_FILTER_VALUE: {
    ...
    case types.NOTES_ADD_ADDITIONAL_FILTER: {
    ...
    case types.NOTES_SET_DROPDOWN_CHECKED: {
    ...
    case types.NOTES_SET_MULTISELECT_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.NOTES_REMOVE_ALL_FILTER: {
      console.log(action.payload);
    ...
    case types.NOTES_REMOVE_QUICK_FILTER: {
    ...
    case types.NOTES_REMOVE_ADVANCED_FILTER: {
      console.log(action.payload);
    ...
    case types.NOTES_QUICK_TEXT_VALUE: {
      console.log(action.payload);
      return {
        ...state,
    ...
    case types.NOTES_RESET_QUICK_FILTER_VALUES: {
      let filterIds = [];
      console.log(action.payload);
    ...
    case types.NOTES_CREATE_NEW_DATASET:
      return {
        ...state,
        newDataset: action.payload,
      };
    case types.NOTES_EDIT_DATASET:
      console.log(action.payload);
      return {
        ...state,
        updateDataSet: action.payload,
      };
    case types.NOTES_SORTED_COLUMN: {
    ...
    case types.NOTES_GET_NEW_DATASET_INFO_SUCCESS: {
    ...
    case types.NOTES_DELETE_DATASET_DB: {
      console.log(action.payload);
    ...
    case types.NOTES_SET_ADV_DROPDOWN_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.NOTES_SET_ADV_MULTISELECT_CHECKED: {
      const { masterid, selectedValue, isChecked } = action.payload;
      console.log(masterid);
      console.log(selectedValue);
      console.log(isChecked);
    ...
    case types.NOTES_SET_NEW_LAYOUT:
      const updatedTotalColumn = [...state.totalColumns];
    ...
    case types.NOTES_SET_COLUMN_SEQ: {
      const updatedTotalColumn = [...state.totalColumns];
      console.log(action.payload);

      // Create a map from action.payload, ensuring displayapiname is valid
      const payloadMap = new Map(
        action.payload
          .filter((item) => item.displayapiname) // Only include items with a valid displayapiname
          .map((item) => [item.displayapiname.toLowerCase(), item])
      );
      ...
      // Update matched items with new sequence and width
      const updatedMatchedItems = matchedItems.map((totalColumnItem) => {
        const payloadItem = action.payload.find(
          (payload) =>
            payload.displayapiname && // Check for valid displayapiname
        ...
        return totalColumnItem; // Return unchanged item if no match
      });
      console.log(updatedMatchedItems);

      // Assign sequence numbers to unmatched items
      let seq =
        action.payload.reduce(
          (maxSeq, item) => Math.max(maxSeq, item.columnsequence || 0), // Handle cases where columnsequence may be null
          0
        ) + 1;
      console.log(seq);
      ...
      const updatedUnmatched = unmatchedItems.map((item) => ({
        ...item,
        seqno: seq++, // Increment sequence number for each unmatched item
      }));
      ...
      // Create a new array for column sequences
      const updatedSeq = action.payload.map((payloadItem) => {
        if (!payloadItem.displayapiname) return null; // Skip items without displayapiname

        const totalColumnItem = updatedTotalColumn.find(
          (totalItem) =>
            totalItem.displayapiname.toLowerCase() ===
            payloadItem.displayapiname.toLowerCase()
        );
      ...
      // Filter out null values from updatedSeq
    ...
    //   const payloadMap = new Map(
    //     action.payload
    //       .filter((item) => item.displayapiname) // Ensure displayapiname exists
    //       .map((item) => [item.displayapiname.toLowerCase(), item])
    //   );
    //   console.log(payloadMap);
    ...
    //   };
    // }
    case types.NOTES_SET_DATEPICKER_VALUE: {
    ...
    case types.NOTES_SET_WEEKFILTER_VALUE: {
    ...
    case types.NOTES_RESET_DATE_FILTERS: {
      console.log("it runs");
      return initialState;
    }
    case types.NOTES_CHANGE_MASTER_DATA: {
    ...
    case types.NOTES_SET_LOADING: {
      return {
        ...state,
        loading: action.payload,
      };
    }
    case types.NOTES_EDIT_CELL: {
    ...
    case types.NOTES_RESET_EDIT_CELL: {
      const resetEditcell = {
