...
const DataSetPopup = ({
  ...
  // const handleEditDataset = (e, datasetId) => {
  //   e.stopPropagation();
  //   // Handle the edit dataset action or popup here
  //   // Example: set datasetId and datasetTitle in the state and open the popup
  //   setSelectedEditDatasetId(datasetId);
  //   const dataset = EntityDataset.entityData.find(
...
function CreateData({
  ...
  const createGroupByData = useSelector(GroupByhorizontalInfo);
  ...
  const selectedgroupbydata = createGroupByData?.map((item) => {
    if (item.api_name === "workweek_Group") {
      return {
        id: item.id,
        type: "Horizontal",
        value: item.grpbyDate,
        dir: "asc",
      };
    } else if (item.api_name === "workday_Group") {
      return {
        id: item.id,
        type: "Horizontal",
        value: item.value,
  ...
  const handleSave = () => {
    if (datasetResult.datasetname.trim() === "") {
      setValidationError("Please enter a Dataset.");
