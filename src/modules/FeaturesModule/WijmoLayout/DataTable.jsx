...
import WijmoPagination from "@/components/FeaturesComp/Wijmo/WijmoPagination";
import { API_TEST_URL, PROTO_ENABLED } from "@/config/serverApiConfig";
  ...
  const sortedColumn = totalColumns
    ...
    const tableData = data?.map((item) => {
      return Object.keys(item).reduce((acc, key) => {
       ...
       else if (lowerCaseKey === "column17") {
          acc["column17_text"] =item[key];
        }
        else if (lowerCaseKey === "column18") {
          acc["column18_text"] =item[key];
        }  else {
