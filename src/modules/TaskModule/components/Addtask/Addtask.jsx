...
import { loginUserIDobjInfo, userobjInfo } from "@/redux/Task/selector";
...
export default function LeadForm({
  ...
  console.log(ProjectPageDetails);
      ...
      value:
        ...
        ["date", "datepicker"].includes(FieldList.controltype)
          ...
          : ["LoggedInUser"].includes(FieldList.default_value)
            ? Globalid
            : ProjectPageDetails?.page === "Project" &&
                FieldList.id === "c78b7183-945c-4c5b-83fb-603e6d3f40fe"
              ? ProjectPageDetails?.columnID
      ...
      return Promise.all(
        sectionlist.fields.map(async (FieldList) => {
          ...
          if (
            ...
            ProjectPageDetails?.page == "Project" &&
            FieldList.default_value == "" &&
            ["onclickloaddropdown"].includes(FieldList.controltype) &&
            FieldList.islookup
          ) {
            console.log(ProjectPageDetails);
            updateOrAddData(FieldList);
            const Options = {
              optionid: ProjectPageDetails?.columnID,
              optionvalue: ProjectPageDetails?.columnValue,
            };
            setInputValues((prevValues) => ({
              ...prevValues,
              [FieldList.api_name + FieldList.id]:
                ProjectPageDetails?.columnID || null,
              [FieldList.api_name + FieldList.id + "_RefLabel"]:
                ProjectPageDetails?.columnValue || null,
  ...
  const handleSaveButtonClick = async () => {
    ...
    if (isValid) {
      ...
      try {
        ...
        console.log("dispatched AddTaskDataValidation");

        dispatch(AddTaskDataValidation(allInputValues)); // Assuming this is async
                                  ...
                                  .map((FieldList, index) => (
                                    <React.Fragment key={index}>
                                      <DynamicInput
                                        ...
                                        Page={
                                          ProjectPageDetails?.page || "AddTask"
                                        }
                                        setArcFilterPopupshow={
                                          setArcFilterPopupshow
                                        }
                                        ...
                                        disabled={
                                          ProjectPageDetails?.disabled || false
                                        }
                                      />
                                    </React.Fragment>
                                  ))}
