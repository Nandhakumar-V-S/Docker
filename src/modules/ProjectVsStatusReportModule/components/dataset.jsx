...
function CreateData({
  ...
  const GroupByhorizontal = useSelector(GroupByhorizontalInfo);
  ...
  const selectedgroupbydata = GroupByhorizontal?.map((item) => {
  ...
  datePickerFilter?.forEach((filter) => {
    newFilterData.push({
      ...
      filtervalue: filter.formattedDate,
      id: filter.id,
      filterelement: filter.api_name,
      filtercategory: "quick",
    });
  ...
  const selectWeekFilter = filterData.filter(
    (filter) => filter.filtercontroltype === "weekfilter"
  );
  console.log(selectWeekFilter);
  if (selectWeekFilter?.length > 0) {
    const data = selectWeekFilter[0];
    console.log(data);
    if (weekFilter.length > 0) {
      weekFilter.forEach((filter) => {
        console.log(filter);
        const value = filter.week + "$" + filter.year;
        newFilterData.push({
          filtertype: "weekfilter",
          filtervalue: filter.week !== "" ? value : "",
          id: filter.id,
          filterelement: filter.api_name,
          filtercategory: "quick",
        });
      ...
      newFilterData.push({
        ...
        filtervalue: "",
        id: data.id,
        filterelement: data.api_name,
        filtercategory: "quick",
      });
    }
  }

  const selectPlanWeekFilter = filterData.filter(
    (filter) => filter.filtercontroltype === "planweekfilter"
  );
  console.log(selectPlanWeekFilter);
  if (selectPlanWeekFilter?.length > 0) {
    const data = selectPlanWeekFilter[0];
    console.log(data);
  ...
  const selectMonthFilter = filterData.filter(
    (filter) => filter.filtercontroltype === "monthfilter"
  );
  if (selectMonthFilter?.length > 0) {
    const data = selectMonthFilter[0];
    if (monthFilter.length > 0) {
      monthFilter?.forEach((filter) => {
        const value = filter.month + "$" + filter.year;
        newFilterData.push({
          filtertype: "monthfilter",
          filtervalue: filter.month !== "" ? value : "",
          id: filter.id,
          filterelement: filter.api_name,
          filtercategory: "quick",
        });
  ...
  const selectPlanMonthFilter = filterData.filter(
    (filter) => filter.filtercontroltype === "planmonthfilter"
  );
  if (selectPlanMonthFilter?.length > 0) {
    const data = selectPlanMonthFilter[0];
    if (planMonthFilter.length > 0) {
      planMonthFilter?.forEach((filter) => {
        const value = filter.month + "$" + filter.year;
        newFilterData.push({
          filtertype: "planmonthfilter",
          filtervalue: filter.month !== "" ? value : "",
          id: filter.id,
          filterelement: filter.api_name,
          filtercategory: "quick",
