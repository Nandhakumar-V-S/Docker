...
export default function Updatesubtask({
  ...
  const { status, DefaultFormValues, error } = useSelector(
    (state) => state.GetsubtaskDtsByTransIdstate
  );
  ...
  useEffect(() => {
    if (TransactionId != "") {
      if (show) dispatch(GetsubtaskDtsByTransId(TransactionId));
    }
  }, [SelectedRow, show]);
  // Fetch data when the component mounts
  useEffect(() => {
    if (show) dispatch(fetchAddSubTaskFormFields());
  }, [show]);
        ...
        updateTaskJson?.result?.groups?.forEach((group) => {
          group.fields?.forEach((field) => {
            ...
            if (field.id.toLowerCase() === defaultValue.id.toLowerCase()) {
              ...
              console.log(field.default_value, "1234");
            }
          });
        });
      });
    ...
    LeadFormAPI?.groups.forEach((sectionlist) => {
      sectionlist.fields.forEach((FieldList) => {
        if (
          ...
          } else if (["dropdown", "multiselect"].includes(FieldList.controltype)) {
