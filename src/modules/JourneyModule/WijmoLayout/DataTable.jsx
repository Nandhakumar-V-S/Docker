...
import WijmoFlexgrid from "@/components/JourneyComp/Wijmo/WijmoFlexgrid";
import WijmoPagination from "@/components/JourneyComp/Wijmo/WijmoPagination";
...
import { setPagination, setPagesize } from "@/redux/Journey/actions";
...
const DataTable = ({ loading }) => {
  ...
  const tableData = data?.map((item) => {
    return Object.keys(item).reduce((acc, key) => {
      ...
      console.log(item);
      const spendHoursValue = item["column4"] || "-";
      console.log(spendHoursValue);
  ...
  useEffect(() => {
    sessionStorage.setItem("360Pagename", "Task");
  }, []);
  useEffect(() => {
