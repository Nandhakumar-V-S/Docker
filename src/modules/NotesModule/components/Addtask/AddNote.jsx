...
function HeaderContent({
  ...
  return (
    <div className="header-content">
      ...
      <h3 title="Add Notes">Add Notes</h3>
...
export default function AddNote({ setArcFilterPopupshow, show, setShow }) {
  ...
  //         if (FieldList.default_value && FieldList.default_value !== null && FieldList.default_value !== "") {
  //           console.log(FieldList);
  //           updateOrAddData(FieldList);
    ...
    LeadFormAPI?.groups?.forEach((sectionlist) => {
      sectionlist.fields.forEach((FieldList) => {
        if (FieldList.default_value && FieldList.default_value !== null) {
          ...
          if (
            FieldList.default_value === "Today" &&
            FieldList.controltype === "datepicker"
          ) {
            setInputValues((prevState) => ({
              ...prevState,
              [FieldList.api_name + FieldList.id]: format(
                new Date(),
                "MM/dd/yyyy"
              ),
            }));
            ...
            FieldList.default_value !== null &&
            FieldList.controltype.includes(["dropdown"])
          ) {
            if (FieldList.default_value === "LoggedInUser") {
              setInputValues((prev) => ({
                ...prev,
                [FieldList.api_name + FieldList.id]: Globalid,
                [FieldList.api_name + FieldList.id + "_RefLabel"]: fullName,
              ...
              console.log(typeof matchedData, "matchedData");

              const masterValues = matchedData
                ? typeof matchedData?.mastervalues === "string"
                  ? JSON?.parse(matchedData?.mastervalues)
                  : matchedData?.mastervalues
                : [];
