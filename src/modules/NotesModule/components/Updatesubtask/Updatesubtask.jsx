...
import AddTaskLocal_API from "@/components/arccomponents/DynamicInputs/AddSubTask-360/AddSubTask-360_API.json";
...
import {
  masterDataInfo as masterDataInfoPlanviewer,
  subMasterDataInfo,
...
import { UpdatesubtaskAPI } from "@/redux/Execution/UpdateSubtask/UpdatesubtaskAPI";
import updateTaskJson from "./updateform.json";
...
import {
  ...
  resetDeaultValues,
} from "@/redux/GetFormFields/GetDefaultFormValues";
...
export default function Updatesubtask({
  ...
  // ! Changed Master Data
  const [TaskDefaultInputs, setTaskDefaultInputs] = useState([]);
  const masterDataTaskPage = useSelector(masterDataInfoTask);
  const masterDataPlanviewerkPage = useSelector(masterDataInfoPlanviewer);
  let location = useLocation();
  const currentPathName = location.pathname;
  const MasterLookups =
    currentPathName === "/task"
      ? masterDataTaskPage
      : masterDataPlanviewerkPage;

  const { status, DefaultFormValues, error } = useSelector(
    (state) => state.DefaultFormValues
  );

  console.log(status, "SampleLen");

  console.log(masterDataTaskPage, "masterDataTaskPage");

  console.log(DefaultFormValues, "DefaultFormValues");
  useEffect(() => {
    setTaskDefaultInputs(DefaultFormValues);
  }, [DefaultFormValues]);
  ...
  const getDefault = async () => {
    const formid = "088C21BD-E246-48DB-8FB7-D7AAD9BAE7DF";
    await dispatch(resetFormFields());
    await dispatch(resetDeaultValues());
    await dispatch(GetFormFields(formid));
    const requestData = {
      endPoint: "/planexecution/taskupadateformdatabyid",
      postData: {
        formid: "088C21BD-E246-48DB-8FB7-D7AAD9BAE7DF",
        transactionid: TransactionId,
      },
    };
    await dispatch(GetDefaultFormValues(requestData));
  };

  // Access the Redux state using useSelector
  // const AddsubTaskFields = useSelector((state) => state.addSubTask);
  const subTaskFields = useSelector((state) => state.FormFields);

  // Extract the relevant data from the state
  // const { screenFields } = AddsubTaskFields;
  const { screenFields } = subTaskFields;
  // const screenFields = updateTaskJson;

  // Access the Redux state using useSelector
  // const TaskDefaultInputs = useSelector(
  //   (state) => state.GetsubtaskDtsByTransId.DefaultFormValues
  // );

  // console.log("hi", AddsubTaskFields);

  //const TaskDefaultInputs = GetsubtaskDtsByTransId.DefaultFormValues;

  // useSelector((state) => state.GetsubtaskDtsByTransIdstate?.DefaultFormValues);
  ...
  useEffect(() => {
        ...
        updatedData?.result?.groups?.forEach((group) => {
          group.fields?.forEach((field) => {
            if (field.id.toLowerCase() === defaultValue.id.toLowerCase()) {
              if (
                ["dropdown", "multiselect"].includes(field.controltype) &&
                field.islookup
              ) {
                (field.default_value =
    ...
    updateDefaultValues();
  }, [TaskDefaultInputs, show, updatedscreenFields]);
  // ! Merge data
        ...
        if (
          ...
          if (
            ...
            if (["LoggedInUser"].includes(FieldList.default_value)) {
              ...
              console.log(masterDataTaskPage, "masterDataTaskPage", FieldList);
              const matchedData = MasterLookups?.find(
  ...
  useEffect(() => {
    ...
    SetDefaultValues();
  }, [TaskDefaultInputs, LeadFormAPI, show, updatedSession]);
  ...
  const handleSaveButtonClick = async () => {
    ...
    if (isValid) {
      ...
      const requestData = {
        entityid: "6A054A51-CAED-4860-9934-754D3EBBAC7D",
        transactionid: TransactionId,
        userid: loggedUserId,
        data: allInputValues,
      };
      setButtonDisabled(true);
      const dipatchedvalue = await dispatch(UpdatesubtaskAPI(requestData));
      const res = await dipatchedvalue?.payload?.result?.response;
      console.log(res);
      if (res === "SubTask Updated successfully.") {
      ...
      // }
      setUpdatedSession(screenFields);
      // setUpdateselectedTagItem([]);
      setUpdateTagItem([]);
    } else {
      setShowErrors(true);
