    ...
    if (matchedFilter) {
      if (matchedFilter.filtercontroltype === "multiselect") {
        ...
        filterValue = selectedOptions
          ...
          ?.join(",");
      } else if (matchedFilter.filtercontroltype === "groupdropdown") {
        // ~ groupdropdown count
        const selectedTagValueLength = selectedFilters.filter(
          (option) => option.filterid === filter.id
        );
        const TotalTags = selectedTagValueLength[0]?.filtervalue;
        const matchedTagFilter = TotalTags;
        console.log(matchedTagFilter);
        // ~ groupdropdown count
        console.log(matchedTagFilter);
        filterValue = matchedTagFilter;
...
function DataSetDelet({ data, handleDeleteDataset }) {
  ...
  return (
    <>
      <span onClick={handleArcPopupShow} title="Delete"> 
        <RiDeleteBin6Line />
      </span>
